...
Quaternion quaternion_normalize(Quaternion q1) {
    ...
    float one_by_sqrt;
    one_by_sqrt =
        InvSqrt(q1.a * q1.a + q1.b * q1.b + q1.c * q1.c + q1.d * q1.d);
...
Quaternion quaternion_between_vectors(vector_ijk v1, vector_ijk v2) {
    ...
    vector_ijk half_way_vector =
        vector_3d_normalize(vector_3d_sum(v1_norm, v2_norm));
...
vector_ijk quaternion_rotate_vector(vector_ijk v, Quaternion q) {
    ...
    Quaternion quaternion_rotated_vector =
        quaternion_product(quaternion_product(q, quaternion_vector), q_inverse);
    vector_ijk rotated = vector_3d_initialize(quaternion_rotated_vector.b,
                                              quaternion_rotated_vector.c,
                                              quaternion_rotated_vector.d);
    return rotated;
}
...
euler_angles quaternion_to_euler_angles(Quaternion q) {
    ...
    double q2 = q.c;
    double q3 = q.d;
    result.roll = atan2(2 * (q0 * q1 + q2 * q3), 1 - 2 * (q1 * q1 + q2 * q2)) *
                  180 / 3.14;
    ...
    if (q.d == 0)
        ...
        result.yaw =
            atan2(2 * (q0 * q3 + q1 * q2), 1 - 2 * (q2 * q2 + q3 * q3)) * 180 /
            3.14;
    return result;
}
