...
_INLINE void I2C_Write_Buffer_16Addr(uint8_t addr, uint8_t reg, uint8_t *data,
                                     uint8_t len) {
  LL_IIC_Write_16addr(addr, reg, data, len);
}

_INLINE void I2C_Read_Buffer_16Addr(uint8_t addr, uint8_t reg, uint8_t *data,
                                    uint8_t len) {
  LL_IIC_Read_16addr(addr, reg, data, len);
}
...
void I2C_Bus_Scan(void) {
  LOG_RAWLN("> I2C Bus Scan Start");
  for (uint8_t i = 1; i < 128; i++) {
    if (I2C_Check_Device(i << 1)) {
      LOG_RAWLN("Found Device on Addr:0x%02X", i);
    }
  }
  LOG_RAWLN("> I2C Bus Scan End");
}

void I2C_Dump_Reg(uint8_t SlaveAddr, uint8_t StartReg, uint8_t StopReg) {
#define _CPL 8
  uint8_t data[_CPL];
  LOG_RAWLN("> I2C/0x%02X Reg Data", SlaveAddr >> 1);
