...
#define CDC_SERIAL_STATE_OVERRUN \
  (1 << 6) /* receive data overrun error has occurred */
...
#define CDC_SERIAL_STATE_TX_CARRIER         \
  (1 << 1) /* state of transmission carrier \
            */
...
struct cdc_call_management_descriptor {
  ...
  uint8_t bDescriptorType; /* CS_INTERFACE descriptor type */
  uint8_t
      bDescriptorSubtype; /* call management functional descriptor subtype */
  uint8_t bmCapabilities; /* capabilities that this configuration supports */
  uint8_t bDataInterface; /* interface number of the data class interface used
                             for call management (optional) */
};

/* Abstract control management functional descriptor */
/* (usbcdc11.pdf, 5.2.3.3) */
/* Describes the command supported by the communication interface class with the
...
/* Describes the relationship between a group of interfaces that can be
