...
usb_osal_thread_t usb_osal_thread_create(const char *name, uint32_t stack_size,
  ...
  aos_task_new_ext(&task_handle, name, entry, args, stack_size,
                   prio + AOS_DEFAULT_APP_PRI - 4);

  return task_handle;
}

void usb_osal_thread_delete(usb_osal_thread_t thread) { aos_task_exit(0); }
...
void usb_osal_mq_delete(usb_osal_mq_t mq) { aos_queue_free((aos_queue_t)mq); }
...
int usb_osal_mq_recv(usb_osal_mq_t mq, uintptr_t *addr, uint32_t timeout) {
  size_t recv_size;
  if (timeout == USB_OSAL_WAITING_FOREVER) {
    return aos_queue_recv((aos_queue_t *)&mq, AOS_WAIT_FOREVER, addr,
                          &recv_size);
  } else {
...
size_t usb_osal_enter_critical_section(void) { return csi_irq_save(); }

void usb_osal_leave_critical_section(size_t flag) { csi_irq_restore(flag); }

void usb_osal_msleep(uint32_t delay) { aos_msleep(delay); }
