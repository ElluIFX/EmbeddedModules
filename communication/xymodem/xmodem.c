...
static xmodem_status xmodem_error_handler(x_uint8_t *error_number,
                                          x_uint8_t max_error_number);
...
int xmodem_receive(void) {
  ...
  while (X_OK == status) {
    ...
    if ((0 != receive_status) && (X_NO_PACKET == x_first_packet_received)) {
      (void)x_transmit_ch(
          X_C);  // 给上位机返回 ACSII "C" ，告诉上位机将使用 CRC-16
    }
    /* 超时或其他错误. */
    else if ((0 != receive_status) &&
             (X_IS_PACKET == x_first_packet_received)) {
    ...
    switch (header[0]) {
      ...
      case X_EOT:
        ...
        x_receive_file_callback(&file_ptr);
        file_ptr = 0;
        return 0;
      /* 发送端发来取消. */
      case X_CAN:
...
static xmodem_status xmodem_handle_packet(x_uint8_t *header) {
  ...
  x_uint8_t received_data[X_PACKET_1024_SIZE + X_PACKET_DATA_INDEX +
                          X_PACKET_CRC_SIZE];
  ...
  if (X_OK == status) {
    ...
    /* 最后两个字节是来自主机的CRC. */
    x_uint16_t crc_received = ((x_uint16_t)received_data[length - 2u] << 8u) |
                              ((x_uint16_t)received_data[length - 1u]);
    /* 校验. */
    x_uint16_t crc_calculated =
...
#endif
      if (x_receive_file_data_callback(
              &file_ptr, (char *)&received_data[X_PACKET_DATA_INDEX], size) !=
          0) {
        /* 硬件错误. */
...
static int get_receive_data(x_uint8_t **data, x_uint32_t len) {
  ...
  while (timeout--)  // 等待数据接收完成
  {
    if (get_recv_len() >= max_len) {
      ...
      if (*data_temp == X_SOH ||
          *data_temp == X_STX)  // 第一个是SOH，说明本次需要接收133个字节
      {
        max_len =
 ...
 * @return  接收到数据的长度.
 */
static x_uint32_t get_recv_len(void) { return recv_len; }
 ...
 * @return  返回写入的结果，0：成功，-1：失败.
 */
__weak int x_receive_file_callback(void **ptr) {
  X_UNUSED(ptr);
