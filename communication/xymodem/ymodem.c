...
static ymodem_status ymodem_error_handler(y_uint8_t *error_number,
                                          y_uint8_t max_error_number);
...
y_uint16_t ymodem_receive(void) {
  ...
  while (Y_OK == status) {
    ...
    /* 用ACSII "C"发送给上位机(直到我们收到一些东西), 告诉上位机我们要使用
     * CRC-16 . */
    if (0 != receive_status) {
      if (Y_NO_PACKET == y_first_packet_received) {
        (void)y_transmit_ch(
            Y_C);  // 给上位机返回 ACSII "C" ，告诉上位机将使用 CRC-16
      } else {
...
static ymodem_status ymodem_handle_packet(y_uint8_t *header) {
  ...
  y_uint8_t received_data[Y_PACKET_1024_SIZE + Y_PACKET_DATA_INDEX +
                          Y_PACKET_CRC_SIZE];
  ...
  if (Y_OK == status) {
    ...
    /* 最后两个字节是来自主机的CRC. */
    y_uint16_t crc_received = ((y_uint16_t)received_data[length - 2u] << 8u) |
                              ((y_uint16_t)received_data[length - 1u]);
    /* 校验. */
    y_uint16_t crc_calculated =
        ymodem_calc_crc(&received_data[Y_PACKET_DATA_INDEX], size);
    ...
    if (255u != (received_data[Y_PACKET_NUMBER_INDEX] +
                 received_data[Y_PACKET_NUMBER_COMPLEMENT_INDEX])) {
      /* 包号和包号补数之和不是255. */
      /* 总和应该总是255. */
      status |= Y_ERROR_NUMBER;
    }

    if (received_data[Y_PACKET_NUMBER_INDEX] == 0x00 &&
        y_first_packet_received == Y_NO_PACKET)  // 第一个包
    {
      ...
      if (strlen(file_name) == 0) {
        ...
        file_len =
            get_file_len((y_uint8_t *)&received_data[Y_PACKET_DATA_INDEX]);
        if (y_receive_nanme_size_callback(&file_ptr, file_name, file_len) !=
            0)  // 调用接收完成回调函数
        {
          /* 硬件错误. */
      ...
      if (file_len == 0xFFFFFFFF)  // 不是有效的长度
      {
        ...
        size =
            get_active_length((y_uint8_t *)&received_data[Y_PACKET_DATA_INDEX],
                              size);  // 获取有效的长度
      }
      ...
      if (y_receive_file_data_callback(
              &file_ptr, (char *)&received_data[Y_PACKET_DATA_INDEX], size) !=
          0) {
        /* 硬件错误. */
        status |= Y_ERROR_FLASH;
      }
...
/**
 * @brief   处理ymodem错误.
 *          增加错误计数器，然后如果错误数量达到临界，则发送终止，否则发送一个
 * NAK.
...
static int get_receive_data(y_uint8_t **data, y_uint32_t len) {
  ...
  while (timeout--)  // 等待数据接收完成
  {
    if (get_recv_len() >= max_len) {
      ...
      if (*data_temp == Y_SOH ||
          *data_temp == Y_STX)  // 第一个是SOH，说明本次需要接收133个字节
      {
        max_len =
            data_len[*data_temp - 1] + 3 + 2;  // 根据不同的头记录不同的长度
      } else {
        break;
      }
    }
 ...
 * @return  void.
 */
static void reset_recv_len(void) { recv_len = 0; }
 ...
 * @return  接收到数据的长度.
 */
static y_uint32_t get_recv_len(void) { return recv_len; }
 ...
 * @return  时间戳
 */
__weak y_uint32_t y_get_tick(void) { return m_time_ms(); }
 ...
 * @return  返回写入的结果，0：成功，-1：失败.
 */
__weak int y_receive_file_callback(void **ptr) {
  Y_UNUSED(ptr);
