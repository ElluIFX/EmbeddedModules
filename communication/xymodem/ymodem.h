...
/* 数据类型定义 */
typedef uint8_t y_uint8_t;
typedef uint16_t y_uint16_t;
typedef uint32_t y_uint32_t;

/* 数据接收缓冲区大小(用户定义,至少要大于 3 + 1024 + 2) */
#define Y_PROT_FRAME_LEN_RECV 1080u
...
/* 0：使用精确时间进行检测，需实现 y_uint32_t y_get_tick(void) 函数，
      该函数应返回毫秒时间戳，当然，也可以是其他值，不过需要调整
   Y_RECEIVE_TIMEOUT 的值

   1：使用循环减 Y_RECEIVE_TIMEOUT
   到数值0的方法，不同频率的开发板可能会需要不同的数值*/
