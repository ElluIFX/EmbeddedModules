...
struct value {
  ...
  union {
    ...
    struct xv (*func)(struct xv value, struct xv args, void *udata);
  };
};
...
static char tmem[XV_THREAD_MEMORY_SIZE];
static int tmemcount = 0;
static size_t tmemused = 0;
static size_t tnumallocs = 0;
static size_t theapsize = 0;
static struct alloc *tallocs = NULL;

static void *emalloc0(size_t sz) { return m_alloc(sz); }

static void efree0(void *ptr) { m_free(ptr); }
...
static struct value undefined(void) { return (struct value){0}; }
...
static struct value err_oom(void) {
  return (struct value){.kind = ERR_KIND, .flag = FLAG_EOOM};
}

static struct value err_undefined(const uint8_t *ident, size_t ilen,
                                  bool chain) {
  return (struct value){
      .kind = ERR_KIND,
    ...
    case JSON_KIND: {
      ...
      if (json_type(json) == JSON_ARRAY) {
        ...
        if (!json_exists(json_next(val))) {
          return to_f64(
              make_json((uint8_t *)json_raw(val), json_raw_length(val)));
        }
      }
      return NAN;
    }
...
static const uint8_t *to_str(struct value a, size_t *len, char buf[],
                             size_t bufsize) {
  if (a.kind == STR_KIND) {
    *len = a.len;
    return a.str;
...
static bool is_err(struct value value) { return value.kind == ERR_KIND; }
...
enum step {
  ...
  STEP_LOGICAL_OR =
      1 << 3,  //  4: Logical OR (||) Nullish coalescing operator (??)
...
static bool string_less(const uint8_t *a, size_t alen, const uint8_t *b,
                        size_t blen) {
    ...
    switch (expr[i]) {
      ...
      case '\'':
        ...
        unreachable(
            ...
            // this group.
            if (!g) return err_syntax();) i = i + glen - 1;
        break;
    }
  }
...
static struct value equal(struct value left, uint8_t op, const uint8_t *expr,
...
static struct value comp(struct value left, uint8_t op, const uint8_t *expr,
                         size_t len, struct eval_context *ctx, int depth) {
  expr = trim(expr, len, &len);
  if (len == 0) return err_syntax();
  // parse next expression
...
static struct value sum(struct value left, uint8_t op, const uint8_t *expr,
                        size_t len, bool neg, struct eval_context *ctx,
                        int depth) {
  expr = trim(expr, len, &len);
  if (len == 0) {
    return err_syntax();
...
static size_t read_id_start(const uint8_t *expr, size_t len, bool *ok) {
  ...
  if (expr[0] == '$' || expr[0] == '_' || (expr[0] >= 'A' && expr[0] <= 'Z') ||
      (expr[0] >= 'a' && expr[0] <= 'z')) {
    *ok = true;
    return 1;
  }
...
static struct value get_ref_value(bool chain, struct value left,
                                  const uint8_t *ident, size_t ilen,
                                  bool opt_chain, struct eval_context *ctx) {
    ...
    if (type == JSON_OBJECT) {
      ...
      while (json_exists(key)) {
        ...
        if (json_string_comparen(key, (char *)ident, ilen) == 0) {
          return make_json((uint8_t *)json_raw(val), json_raw_length(val));
        }
        key = json_next(val);
      }
    } else {  // JSON_ARRAY
  ...
  struct value val = to_value(
      ctx->env->ref(from_value(chain ? left : make_global()),
...
static struct value eval_foreach(const uint8_t *expr, size_t len,
                                 struct xv_env *env,
                                 void (*iter)(struct value, void *udata),
...
static struct value multi_exprs_to_array(const uint8_t *expr, size_t len,
  ...
  struct value last =
      eval_foreach(expr, len, ctx->env, multi_iter, &ictx, depth);
...
static struct value eval_atom(const uint8_t *expr, size_t len,
  ...
  if (!left_ready) {
    ...
    if (ilen == 4 && memcmp(ident, "true", 4) == 0) {
      ...
      left = get_ref_value(false, make_undefined(), ident, ilen, false, ctx);
      if (is_err(left)) return left;
    }
    left_ready = true;
    expr = expr + ilen;
  ...
  while (1) {
    ...
    switch (expr[0]) {
      ...
      case '(':
        ...
        val = to_value(left.func(from_value(left_left), from_value(args),
  ...
  for (size_t i = 0; i < len; i++) {
    switch (expr[i]) {
      ...
      case '+':
        ...
        if (neg) {
          if (s > 0 && s < len && expr[s - 1] == '-' && expr[s] >= '0' &&
              expr[s] <= '9') {
            s--;
            neg = false;
          }
...
static struct value eval(const uint8_t *expr, size_t len, struct xv_env *env,
                         int depth) {
...
double xv_double(struct xv value) { return to_f64(to_value(value)); }

int64_t xv_int64(struct xv value) { return to_i64(to_value(value)); }

uint64_t xv_uint64(struct xv value) { return to_u64(to_value(value)); }

bool xv_bool(struct xv value) { return to_bool(to_value(value)); }
...
static int64_t conv_ttoi(bool t) { return t ? 1 : 0; }

// conv_ttou converts bool to uint64
static uint64_t conv_ttou(bool t) { return t ? 1 : 0; }

// conv_ttof converts bool to double
static double conv_ttof(bool t) { return t ? 1 : 0; }
...
static bool conv_ftot(double f) { return f < 0 || f > 0; }
...
static bool conv_itot(int64_t i) { return i != 0; }

// conv_itof converts int64 to double
static double conv_itof(int64_t i) { return (double)i; }
...
static bool conv_utot(uint64_t u) { return u != 0; }

// Utof converts uint64 to double
static double conv_utof(uint64_t u) { return (double)u; }
...
static bool isnumch(char c) { return (c >= '0' && c <= '9') || c == '.'; }
...
static double conv_atof(const char *a, size_t alen) {
  ...
  } else if (alen == 8 && memcmp(a, "Infinity", 8) == 0)
    return INFINITY;
  else if (alen == 9 && memcmp(a, "+Infinity", 9) == 0)
    return INFINITY;
  else if (alen == 9 && memcmp(a, "-Infinity", 9) == 0)
    return -INFINITY;
  return NAN;
...
struct xv xv_new_double(double d) { return from_value(make_float(d)); }

struct xv xv_new_int64(int64_t i) { return from_value(make_int(i)); }

struct xv xv_new_uint64(uint64_t u) { return from_value(make_uint(u)); }

struct xv xv_new_boolean(bool t) { return from_value(make_bool(t)); }

struct xv xv_new_undefined(void) { return from_value(make_undefined()); }

struct xv xv_new_null(void) { return from_value(make_null()); }
...
struct xv xv_new_error(const char *msg) { return from_value(err_msg(msg)); }
...
bool xv_is_error(struct xv value) { return is_err(to_value(value)); }
