...
#ifndef T
#error "Template type T undefined for <./bits/iterators.h>"
#endif
...
#define list_ctl_foreach(A, self, pos)                             \
  if ((self)->size)                                                \
    for (JOIN(A, it) pos = JOIN(A, begin)(self); pos.node != NULL; \
         pos.node = JOIN(JOIN(A, node), next)(pos.node))
#define list_ctl_foreach_range(A, pos, first, last)            \
  if (first && first->node)                                    \
    for (JOIN(A, it) pos = *(first); pos.node != (last)->node; \
         pos.node = JOIN(JOIN(A, node), next)(pos.node))
...
#define vec_ctl_foreach(T, self, ref)                                      \
  if ((self)->size)                                                        \
    for (T *ref = &(self)->vector[0]; ref < &(self)->vector[(self)->size]; \
         ref++)
#define vec_ctl_foreach_range(T, self, it, first, last) \
...
#define ctl_foreach(A, self, pos)                                              \
  for (JOIN(A, it) pos = JOIN(A, begin)(self); !JOIN(JOIN(A, it), done)(&pos); \
       JOIN(JOIN(A, it), next)(&pos))
#define ctl_foreach_range(A, pos, first, last) \
  JOIN(A, it) pos = *first;                    \
  JOIN(JOIN(A, it), range)(&pos, last);        \
  for (; !JOIN(JOIN(A, it), done)(&pos); JOIN(JOIN(A, it), next)(&pos))
#define ctl_foreach_range_(A, pos, first) \
  JOIN(A, it) pos = *first;               \
  for (; !JOIN(JOIN(A, it), done)(&pos); JOIN(JOIN(A, it), next)(&pos))

#define ctl_foreach_n(A, self, pos, n)                      \
  ...
  for (; !JOIN(JOIN(A, it), done)(&pos); JOIN(JOIN(A, it), next)(&pos))
#define ctl_foreach_n_range(A, first, pos, n)             \
  JOIN(A, it) pos = *first;                               \
  {                                                       \
