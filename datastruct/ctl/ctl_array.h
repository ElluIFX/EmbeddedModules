...
static inline size_t JOIN(A, max_size)(void) { return N; }
...
static inline T *JOIN(A, back)(A *self) { return &self->vector[N - 1]; }

static inline I JOIN(A, begin)(A *self) { return JOIN(I, iter)(self, 0); }

static inline I JOIN(A, end)(A *self) { return JOIN(I, iter)(self, N); }

static inline T *JOIN(I, ref)(I *iter) { return iter->ref; }

static inline size_t JOIN(I, index)(I *iter) {
  return iter->ref - JOIN(A, front)(iter->container);
}

static inline int JOIN(I, done)(I *iter) { return iter->ref == iter->end; }

static inline void JOIN(I, set_done)(I *iter) { iter->ref = iter->end; }

static inline void JOIN(I, set_pos)(I *iter, I *other) {
  iter->ref = other->ref;
}

static inline void JOIN(I, next)(I *iter) { iter->ref++; }

static inline void JOIN(I, prev)(I *iter) { iter->ref--; }

static inline void JOIN(I, range)(I *first, I *last) {
  last->end = first->end = last->ref;
}

static inline void JOIN(I, set_end)(I *iter, I *last) { iter->end = last->ref; }
...
#ifndef POD
  if (self->free && !JOIN(A, zero)(ref)) self->free(ref);
#endif
  *ref = value;
}
...
static inline void JOIN(A, fill)(A *self, T value) {
  ...
  if (sizeof(T) <= sizeof(char))  // only for bytes
    ...
    for (size_t i = 0; i < N; i++) self->vector[i] = value;
#else
  for (size_t i = 0; i < N; i++) JOIN(A, set)(self, i, self->copy(&value));
#endif
#ifndef POD
...
static inline void JOIN(A, fill_range)(I *range, T value) {
  ...
  if (sizeof(T) <= sizeof(char))  // only for bytes
    ...
    for (I it = *range; it.ref < it.end; it.ref++) *it.ref = value;
#else
...
#endif
  for (I it = *range; it.ref < it.end; it.ref++) *it.ref = zero;
}
...
static inline void JOIN(A, assign)(A *self, size_t count, T value) {
  for (size_t i = 0; i < count; i++) JOIN(A, set)(self, i, self->copy(&value));
#ifndef POD
  if (self->free) self->free(&value);
#endif
}
...
static inline T *JOIN(A, data)(A *self) { return JOIN(A, front)(self); }
...
static inline void JOIN(A, _ranged_sort)(A *self, size_t a, size_t b,
  ...
  if (LIKELY(z)) JOIN(A, _ranged_sort)(self, a, z - 1, _compare);
  // check overflow of z + 1
  if (LIKELY(z + 1 > z)) JOIN(A, _ranged_sort)(self, z + 1, b, _compare);
}

static inline void JOIN(A, sort)(A *self) {
  CTL_ASSERT_COMPARE
  // TODO insertion_sort cutoff
  if (LIKELY(N > 1)) JOIN(A, _ranged_sort)(self, 0, N - 1, self->compare);
}
...
static inline T *JOIN(A, find)(A *self, T key) {
  ctl_foreach(A, self, it)
    if (JOIN(A, _equal)(self, it.ref, &key)) return it.ref;
  return NULL;
}
...
static inline A JOIN(A, copy_if_range)(I *range, int _match(T *)) {
  ...
  while (!JOIN(I, done)(range)) {
    if (_match(range->ref)) JOIN(A, set)(&out, i, out.copy(range->ref));
    i++;
    JOIN(I, next)(range);
  }
  return out;
...
static inline I JOIN(A, difference_range)(I *r1, I *r2) {
  ...
  *self = JOIN(A, init)();
  ...
  while (!JOIN(I, done)(r1)) {
    if (done2(r2)) return JOIN(A, copy_range)(&it, r1);
    ...
    if (self->compare(r1->ref, ref2(r2))) {
      *it.ref = self->copy(r1->ref);
      ...
      if (!self->compare(ref2(r2), r1->ref)) JOIN(I, next)(r1);
      next2(r2);
    }
  }
  // JOIN(A, fill_zero)(&it);
