...
static inline B **JOIN(A, first)(A *self) { return &self->pages[self->mark_a]; }
...
static inline T *JOIN(A, at)(A *self, size_t index) {
  ...
  if (index == self->size)
    ...
    if (!self->pages) return NULL;
    self->pages[0] = (B *)CTL_CALLOC(1, sizeof(B));
    if (!self->pages[0]) return NULL;
...
static inline T *JOIN(A, front)(A *self) { return JOIN(A, at)(self, 0); }

static inline T *JOIN(A, back)(A *self) {
  return self->size ? JOIN(A, at)(self, self->size - 1) : NULL;
}

static inline I JOIN(A, begin)(A *self) { return JOIN(B, iter)(self, 0); }
...
static inline size_t JOIN(I, index)(I *iter) { return iter->index; }

static inline int JOIN(I, done)(I *iter) { return iter->index == iter->end; }

static inline void JOIN(I, set_done)(I *iter) { iter->index = iter->end; }
...
static inline B *JOIN(B, next)(B *self) { return self; }
...
static inline I *JOIN(A, erase_range)(I *range) {
  ...
  size_t e = range->end;
  if (i >= self->size || i >= e) return range;
  for (; i < e; e--) JOIN(A, erase_index)(self, i);
  return range;
}
...
static inline void JOIN(A, assign)(A *self, size_t size, T value) {
  ...
  for (size_t i = 0; i < size; i++) JOIN(A, set)(self, i, self->copy(&value));
  if (self->free) self->free(&value);
}
...
#ifndef POD
      if (self->free && i < orig_size && sref) self->free(sref);
#endif
...
static inline void JOIN(A, _ranged_sort)(A *self, size_t from, size_t to,
  ...
  if (LIKELY(z)) JOIN(A, _ranged_sort)(self, from, z - 1, _compare);
  // check overflow of z + 1
  if (LIKELY(z + 1 > z)) JOIN(A, _ranged_sort)(self, z + 1, to, _compare);
}
...
static inline I JOIN(A, find)(A *self, T key) {
  ctl_foreach(A, self, i)
    if (JOIN(A, _equal)(self, i.ref, &key)) return i;
