...
static inline int JOIN(A, empty)(A *self) { return self->head == NULL; }

static inline T *JOIN(A, front)(A *self) { return &self->head->value; }
...
static inline void JOIN(I, next)(I *iter) {
  if (LIKELY(iter->node)) {
    ...
    if (LIKELY(iter->node)) iter->ref = &iter->node->value;
  }
}
...
static inline void JOIN(I, advance_end)(I *iter, long n) {
  B *node = iter->node;
  if (node)
    for (long i = 0; i < n && node != iter->end; i++) node = node->next;
  iter->end = iter->node;
}
...
static inline size_t JOIN(A, size)(A *self) {
  size_t size = 0;
  for (B *n = self->head; n; size++) n = n->next;
  return size;
}
...
static inline I JOIN(I, iter)(A *self, B *begin) {
  ...
  I iter = zero;
  iter.node = begin;
  if (LIKELY(begin)) iter.ref = &begin->value;
  iter.container = self;
...
static inline B *JOIN(A, erase_after)(A *self, B *node) {
  ...
  if (node->next) node->next = JOIN(B, erase_node)(self, node->next);
  return node;
}
...
static inline void JOIN(B, insert_after)(B *position, T value) {
  ...
  if (next) position->next->next = next;
}
...
static inline void JOIN(A, clear)(A *self) {
  while (!JOIN(A, empty)(self)) JOIN(A, pop_front)(self);
}
...
static inline void JOIN(A, assign)(A *self, size_t size, T value) {
  ...
  // too large: shrink
  while (node) node = JOIN(A, erase_after)(self, node);
...
static inline int JOIN(A, equal)(A *self, A *other) {
  if (JOIN(A, empty)(self)) return JOIN(A, empty)(other);
  I a = JOIN(A, begin)(self);
  I b = JOIN(A, begin)(other);
  while (!JOIN(I, done)(&a) && !b.vtable.done(&b)) {
    if (!self->equal(a.ref, b.vtable.ref(&b))) return 0;
    JOIN(I, next)(&a);
    b.vtable.next(&b);
  }
  return 1;
...
static inline A JOIN(A, merge_range)(I *r1, GI *r2) {
  ...
  */
  ...
  while (!JOIN(I, done)(r1)) {
    B *next;
    if (done2(r2)) return *JOIN(A, copy_range)(r1, &self);
...
static inline void JOIN(A, sort_range)(I *range) {
      ...
      range->node != range->end)  // have tail behind range->end
  {
    B *last = range->node;
    B *tail = range->end;
    while (last->next != range->end) last = last->next;
    last->next = NULL;
...
static inline A JOIN(A, union_range)(I *r1, GI *r2) {
  ...
  while (!JOIN(I, done)(r1)) {
    ...
    if (self.compare(ref2(r2), r1->ref)) {
      ...
      if (!self.compare(r1->ref, ref2(r2))) next2(r2);
      JOIN(I, next)(r1);
    }
  }
