...
static inline B *JOIN(A, first)(A *self) { return JOIN(B, min)(self->root); }
...
static inline B *JOIN(A, last)(A *self) { return JOIN(B, max)(self->root); }
...
static inline void JOIN(I, advance_end)(I *iter, long n) {
  B *node = iter->node;
  for (long j = 0; node != NULL && j < n; j++) node = JOIN(B, next)(node);
  iter->end = node;
}

static inline long JOIN(I, distance)(I *iter, I *other) {
  long d = 0;
  if (iter == other || iter->node == other->node) return 0;
  B *n = iter->node;
  for (; n != NULL && n != other->node; d++) n = JOIN(B, next)(n);
  if (n == other->node) return d;
  // other before iter, negative result
  d = (long)other->container->size;
  n = other->node;
  for (; n != NULL && n != iter->node; d--) n = JOIN(B, next)(n);
...
static inline size_t JOIN(I, distance_range)(I *range) {
  ...
  for (; n != NULL && n != range->end; d++) n = JOIN(B, next)(n);
  return d;
}
...
static inline int JOIN(B, color)(B *node) { return node ? node->color : 1; }

static inline int JOIN(B, is_black)(B *node) {
  return JOIN(B, color)(node) == 1;
}

static inline int JOIN(B, is_red)(B *node) { return JOIN(B, color)(node) == 0; }

static inline B *JOIN(B, grandfather)(B *node) { return node->p->p; }
...
static inline B *JOIN(B, uncle)(B *node) { return JOIN(B, sibling)(node->p); }
...
static inline void JOIN(A, erase_it)(I *it) {
  B *node = it->node;
  if (node) JOIN(A, erase_node)(it->container, node);
}
...
static inline void JOIN(A, erase)(A *self, T key) {
  B *node = JOIN(A, find_node)(self, key);
  if (node) JOIN(A, erase_node)(self, node);
}

static inline void JOIN(A, erase_1)(A *self, B *node) {
  if (node->p) JOIN(A, erase_2)(self, node);
}
...
static inline void JOIN(A, clear)(A *self) {
  while (!JOIN(A, empty)(self)) JOIN(B, erase_fast)(self, self->root);
  self->root = NULL;
}
...
static inline void JOIN(A, generate_n)(A *self, size_t count, T _gen(void)) {
  for (size_t i = 0; i < count; i++) JOIN(A, insert)(self, _gen());
}
...
static inline bool JOIN(A, find_first_of_range)(I *range1, GI *range2) {
  ...
  if (JOIN(I, done)(range1) || done2(range2)) return false;
  I it = *range2;
  while (1) {
    if (done2(&it)) goto not_found;
