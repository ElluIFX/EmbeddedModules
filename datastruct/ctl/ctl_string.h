...
#define str_ctl_foreach(self, it)                                           \
  if ((self)->size)                                                         \
    for (char *it = &(self)->vector[0]; it < &(self)->vector[(self)->size]; \
         it++)
...
static inline char *str_begin(str *self) { return &self->vector[0]; }

static inline char *str_end(str *self) { return &self->vector[self->size]; }

// if we compare char by char, for algorithms like sort
static inline int str_char_compare(char *a, char *b) { return *a < *b; }

static inline int str_char_equal(char *a, char *b) { return *a == *b; }
...
static inline str str_init(const char *c_str) {
  ...
  for (const char *s = c_str; *s; s++) str_push_back(&self, *s);
  return self;
}
...
static inline void str_append(str *self, const char *s) {
  ...
  for (size_t i = 0; i < len; i++) self->vector[start + i] = s[i];
}
...
static inline void str_replace(str *self, size_t index, size_t size,
  ...
  for (size_t i = index; i < end; i++) str_erase_index(self, index);
  str_insert_str(self, index, s);
}

static inline char *str_c_str(str *self) { return str_data(self); }
...
static inline bool str_find_first_of_range(str_it *range1, str_it *range2) {
  if (str_it_done(range1) || str_it_done(range2)) return false;
...
static inline str str_substr(str *self, size_t index, size_t size) {
  ...
  for (size_t i = 0; i < size; i++) substr.vector[i] = self->vector[index + i];
  return substr;
}
