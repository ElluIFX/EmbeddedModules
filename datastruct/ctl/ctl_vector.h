...
static inline size_t JOIN(A, capacity)(A *self) { return self->capacity; }
...
static inline void JOIN(A, wipe)(A *self, size_t n) {
  ...
  if (self->capacity <= 30) JOIN(A, fit)(self, 47);
#endif
}

static inline void JOIN(A, clear)(A *self) {
  if (self->size > 0) JOIN(A, wipe)(self, self->size);
}
...
static inline void JOIN(A, reserve)(A *self, const size_t n) {
...
#endif
  {
    ...
    if (actual > 0) {
      ...
      {
        if (actual < 2 * self->capacity) actual = 2 * self->capacity;
        if (actual > max_size) actual = max_size;
...
static inline void JOIN(A, push_back)(A *self, T value) {
  if (self->size == self->capacity) JOIN(A, reserve)
...
static inline void JOIN(A, resize)(A *self, size_t size, T value) {
  if (size < self->size) {
    ...
    if (less > 0) JOIN(A, wipe)(self, less);
  } else {
...
static inline void JOIN(A, assign_range)(A *self, T *from, T *last) {
  ...
  while (from != last) {
    if (i >= orig_size)  // grow
      ...
      T *ref = &self->vector[i];
      if (self->free && i < orig_size) self->free(ref);
      *ref = self->copy(from);
    }
    from++;
    i++;
  ...
  if (i < orig_size)  // shrink
    while (i != self->size) JOIN(A, pop_back)(self);
}
...
static inline void JOIN(A, assign_generic)(A *self, GI *range) {
  ...
  while (!done(range)) {
    if (i >= orig_size)  // grow
      ...
      if (self->free && i < orig_size) self->free(sref);  // replace
...
static inline I JOIN(A, erase_index)(A *self, size_t index) {
  static T zero;
#if 1
  if (self->free) self->free(&self->vector[index]);
...
static inline I *JOIN(A, erase_range)(I *range) {
  if (JOIN(I, done)(range)) return range;
...
#ifndef POD
  if (self->free)
    for (T *ref = range->ref; ref < range->end; ref++) self->free(ref);
#endif
  if (range->end != end)
...
#ifndef CTL_STR
static inline I JOIN(A, find)(A *self, T key) {
  vec_ctl_foreach(T, self,
                  ref) if (JOIN(A, _equal)(self, ref, &key)) return JOIN(I,
                                                                         iter)(
