...
#ifndef __OPTIMIZE__
#pragma message \
    "fifofast.h is intended to be compiled with optimisation and will run VERY SLOWLY without!"
#endif
...
#define _FFF_SIZEOF_DATA(_id) \
    sizeof(((struct _FFF_NAME_STRUCT(_id)*)0)->data[0])
#define _FFF_SIZEOF_ARRAY(_id) \
    _sizeof_array(((struct _FFF_NAME_STRUCT(_id)*)0)->data)

// returns the length of the data array; fifo constrains are automatically applied
#define _FFF_GET_ARRAYDEPTH(_depth) \
    _limit(ROUND_UP_2N(_depth), 4, ((uint32_t)1 << 31))
#define _FFF_GET_ARRAYDEPTH_P(_depth) \
    _limit(ROUND_UP_2N(_depth), 4, ROUND_UP_2N(FIFOFAST_MAX_DEPTH_POINTABLE))
...
#define FIFOFAST_INDEX_T \
    _type_min(ROUND_UP_2N(FIFOFAST_MAX_DEPTH_POINTABLE) - 1)
...
typedef struct {
    ...
    fff_level_t
        level;  // current amount of stored data. Is larger than 'mask', if full
...
static inline void* fff_peek_read(fff_proto_t* fifo, fff_index_t idx)
    __attribute__((__always_inline__));
static inline void fff_peek_write(fff_proto_t* fifo, fff_index_t idx,
                                  void* data)
    __attribute__((__always_inline__));
...
#define _fff_declare_a(_type, _id, _depth, _size) \
    _fff_declare(_type, _id, _depth)[_size]
#define _fff_declare_pa(_type, _id, _depth, _size) \
    _fff_declare_p(_type, , _depth)[_size]
...
#define _fff_init_a(_id, _arraysize) \
    struct _FFF_NAME_STRUCT(_id)     \
        _id[] = {[0 ... _arraysize - 1] = {0, 0, 0, {}}}

#define _fff_init_p(_id)         \
    struct _FFF_NAME_STRUCT(_id) \
        _id = {_FFF_SIZEOF_DATA(_id), _FFF_SIZEOF_ARRAY(_id) - 1, 0, 0, 0, {}}

#define _fff_init_pa(_id, _arraysize)      \
    struct _FFF_NAME_STRUCT(_id) _id[] = { \
        [0 ... _arraysize - 1] = {         \
            _FFF_SIZEOF_DATA(_id), _FFF_SIZEOF_ARRAY(_id) - 1, 0, 0, 0, {}}}
...
static inline void fff_peek_write(fff_proto_t* fifo, fff_index_t idx,
                                  void* data) {
    memcpy(fff_data_p(fifo, fff_wrap(fifo, fifo->read + idx)), data,
           fifo->data_size);
}
