...
bool SPIF_FindChip(SPIF_HandleTypeDef* Handle) {
    ...
    do {
        ...
        Handle->PageCnt =
            (Handle->SectorCnt * SPIF_SECTOR_SIZE) / SPIF_PAGE_SIZE;
...
bool SPIF_ReadFn(SPIF_HandleTypeDef* Handle, uint32_t Address, uint8_t* Data,
    ...
    do {
        ...
        SPIF_CsPin(Handle, 1);
        SDEBUG("SPIF_ReadAddress() %d BYTES READ DONE AFTER %ld ms",
               (uint16_t)Size, SPIF_Tick() - dbgTime);
        retVal = true;
...
bool SPIF_EraseSector(SPIF_HandleTypeDef* Handle, uint32_t Sector) {
    ...
    do {
        ...
        if (SPIF_WaitForWriting(Handle, 1000)) {
            SDEBUG("SPIF_EraseSector() DONE AFTER %ld ms",
                   SPIF_Tick() - dbgTime);
            retVal = true;
        }
...
bool SPIF_EraseBlock(SPIF_HandleTypeDef* Handle, uint32_t Block) {
    ...
    do {
        ...
        if (SPIF_WaitForWriting(Handle, 3000)) {
            SDEBUG("SPIF_EraseBlock() DONE AFTER %ld ms",
                   SPIF_Tick() - dbgTime);
            retVal = true;
        }
...
bool SPIF_WriteSector(SPIF_HandleTypeDef* Handle, uint32_t SectorNumber,
    ...
    do {
        ...
        while (remainingBytes > 0 &&
            ...
            uint32_t bytesToWrite = (remainingBytes > SPIF_PAGE_SIZE)
                                        ? SPIF_PAGE_SIZE
                                        : remainingBytes;
            if (SPIF_WriteFn(Handle, pageNumber, Data + bytesWritten,
                             bytesToWrite, pageOffset) == false) {
                retVal = false;
                break;
            }
