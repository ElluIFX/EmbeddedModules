...
bool embeddedCliDelBinding(EmbeddedCli *cli, const char *name) {
  PREPARE_IMPL(cli);
  if (impl->bindingsCount == 0) return false;
...
static void onEscapedInput(EmbeddedCli *cli, char c) {
  ...
  if (c >= 64 && c <= 126) {
    ...
    if (c == 'A' || c == 'B') {
      ...
      // treat \e[..C as cursor right and \e[..D as cursor left
    }
  }
}
...
static void parseCommand(EmbeddedCli *cli) {
  ...
  for (int i = 0; i < impl->bindingsCount; ++i) {
    if (strcmp(cmdName, impl->bindings[i].name) == 0) {
      if (impl->bindings[i].func == NULL) break;
      ...
      // set)
      if (cmdArgs != NULL &&
          (strcmp(cmdArgs, "-h") == 0 || strcmp(cmdArgs, "--help") == 0)) {
