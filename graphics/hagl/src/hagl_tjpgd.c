        ...
        for (i = 0; i < 64; i++) { /* Load the table */
            ...
            pb[z] =
                (int32_t)((uint32_t)*data++ *
                          IPSF(
                              z)); /* Apply scale factor of Arai algorithm to the
                                     de-quantizers */
        }
    }
...
static int create_huffman_tbl(          /* 0:OK, !0:Failed */
    ...
    while (ndata) { /* Process all tables in the segment */
        ...
        ph = alloc_pool(
            jd,
            (uint16_t)(np * sizeof(uint16_t))); /* Allocate a memory block for
                                                   the code word table */
        if (!ph)
            return JDR_MEM1; /* Err: not enough memory */
        ...
        if (!msk) {    /* Next byte? */
            ...
            if (f) {   /* In flag sequence? */
                ...
                if (*dp != 0)
                    return 0 -
                           (int16_t)
                               JDR_FMT1; /* Err: unexpected flag is detected (may
                                           be collapted data) */
...
static JRESULT mcu_load(JDEC* jd /* Pointer to the decompressor object */
    ...
    for (blk = 0; blk < nby + nbc; blk++) {
        ...
        hd = jd->huffdata[id][0];
        b = huffext(jd, hb, hc,
                    hd); /* Extract a huffman coded data (bit length) */
        if (b < 0)
        ...
        dqf =
            jd->qttbl
                [jd->qtid[cmp]];  /* De-quantizer table ID for this component */
        ...
        do {
            ...
            if (b &= 0x0F) {       /* Bit length */
                ...
                tmp[z] =
                    d * dqf[z] >> 8; /* De-quantize, apply scale factor of Arai
                                     algorithm and descale 8 bits */
            }
        ...
        if (JD_USE_SCALE && jd->scale == 3) {
            *bp =
            ...
            block_idct(
                tmp, bp); /* Apply IDCT and store the block to the MCU buffer */
        }
...
static JRESULT mcu_output(
    ...
    if (!JD_USE_SCALE || jd->scale != 3) { /* Not for 1/8 scaling */
        ...
        for (iy = 0; iy < my; iy++) {
            ...
            for (ix = 0; ix < mx; ix++) {
                cb = pc[0] -
                     128; /* Get Cb/Cr component and restore right level */
                cr = pc[64] - 128;
                if (mx == 16) { /* Double block width? */
                    if (ix == 8)
                        py += 64 -
                              8;  /* Jump to next block if double block heigt */
                ...
                *rgb24++ =
                    /* R */ BYTECLIP(yy +
                                     ((int16_t)(1.402 * CVACC) * cr) / CVACC);
                *rgb24++ =
                    /* G */ BYTECLIP(yy - ((int16_t)(0.344 * CVACC) * cb +
                                           (int16_t)(0.714 * CVACC) * cr) /
                                              CVACC);
                *rgb24++ =
                    /* B */ BYTECLIP(yy +
                                     ((int16_t)(1.772 * CVACC) * cb) / CVACC);
            }
        }
    ...
    for (;;) {
        ...
        switch (marker & 0xFF) {
            case 0xC0: /* SOF0 (baseline JPEG) */
                ...
                for (i = 0; i < 3; i++) {
                    ...
                    b = seg
                        [8 +
                         3 * i]; /* Get dequantizer table ID for this component */
                    if (b > 3)
         ...
         */
                for (i = 0; i < 3; i++) {
                    ...
                    if (!jd->huffbits[b][0] ||
                        !jd->huffbits
                             [b][1]) {   /* Check dc/ac huffman table for this
                                        component */
                        return JDR_FMT1; /* Err: Nnot loaded */
                    }
                    if (!jd->qttbl
                             [jd->qtid
                                  [i]]) { /* Check dequantizer table for this
                                            component */
                ...
                jd->mcubuf = (uint8_t*)alloc_pool(
                    jd,
                    (uint16_t)((n + 2) * 64)); /* Allocate MCU working buffer */
                if (!jd->mcubuf)
                    return JDR_MEM1; /* Err: not enough memory */
                ...
                if (ofs %= JD_SZBUF) { /* Align read offset to JD_SZBUF */
                    jd->dctr =
                        jd->infunc(jd, seg + ofs, (uint16_t)(JD_SZBUF - ofs));
                    jd->dptr = seg + ofs - 1;
                }
...
JRESULT jd_decomp(JDEC* jd, /* Initialized decompression object */
    ...
    for (y = 0; y < jd->height; y += my) {    /* Vertical loop of MCUs */
        for (x = 0; x < jd->width; x += mx) { /* Horizontal loop of MCUs */
            ...
            rc = mcu_load(
                jd); /* Load an MCU (decompress huffman coded stream and
                            apply IDCT) */
            if (rc != JDR_OK)
                return rc;
            rc = mcu_output(
