      ...
      if (rd_len == rd_target) {
        ...
        switch (type) {
          case VLCD_INPKT_TYPE_KEYBOARD: {
            vlcd_inpkt_keyboard_t* pkt = (vlcd_inpkt_keyboard_t*)buf;
            vlcd_keyboard_callback(pkt->action, pkt->scancode, pkt->modifier,
                                   pkt->ascii);
            break;
          }
          ...
          case VLCD_INPKT_TYPE_MOUSE: {
            ...
            vlcd_mouse_callback(pkt->x, pkt->y, pkt->wheel, pkt->mousekey);
            break;
          }
