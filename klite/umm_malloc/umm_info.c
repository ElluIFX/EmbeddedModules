...
void compute_usage_metric(void) {
  if (0 == ummHeapInfo.freeBlocks) {
    ...
    ummHeapInfo.usage_metric =
        (int)((ummHeapInfo.usedBlocks * 100) / (ummHeapInfo.freeBlocks));
  }
}
...
void compute_fragmentation_metric(void) {
  if (0 == ummHeapInfo.freeBlocks) {
    ummHeapInfo.fragmentation_metric =
        0;  // No free blocks ... so no fragmentation either!
  } else {
    ummHeapInfo.fragmentation_metric =
        100 - (((uint32_t)(sqrtf(ummHeapInfo.freeBlocksSquared)) * 100) /
...
void *umm_multi_info(umm_heap *heap, void *ptr, bool force) {
  ...
  DBGLOG_FORCE(force, "|0x%08x|B %5i|NB %5i|PB %5i|Z %5i|NF %5i|PF %5i|\n",
               DBGLOG_32_BIT_PTR(&UMM_BLOCK(blockNo)), blockNo,
               UMM_NBLOCK(blockNo) & UMM_BLOCKNO_MASK, UMM_PBLOCK(blockNo),
  ...
  while (UMM_NBLOCK(blockNo) & UMM_BLOCKNO_MASK) {
    ...
    if (UMM_NBLOCK(blockNo) & UMM_FREELIST_MASK) {
      ...
      DBGLOG_FORCE(force, "|0x%08x|B %5i|NB %5i|PB %5i|Z %5u|NF %5i|PF %5i|\n",
                   DBGLOG_32_BIT_PTR(&UMM_BLOCK(blockNo)), blockNo,
                   UMM_NBLOCK(blockNo) & UMM_BLOCKNO_MASK, UMM_PBLOCK(blockNo),
                   (uint16_t)curBlocks, UMM_NFREE(blockNo), UMM_PFREE(blockNo));
  ...
  DBGLOG_FORCE(force, "|0x%08x|B %5i|NB %5i|PB %5i|Z %5i|NF %5i|PF %5i|\n",
               DBGLOG_32_BIT_PTR(&UMM_BLOCK(blockNo)), blockNo,
               UMM_NBLOCK(blockNo) & UMM_BLOCKNO_MASK, UMM_PBLOCK(blockNo),
               UMM_NUMBLOCKS - blockNo, UMM_NFREE(blockNo), UMM_PFREE(blockNo));
  ...
  DBGLOG_FORCE(force,
               ...
               ummHeapInfo.totalEntries, ummHeapInfo.usedEntries,
               ummHeapInfo.freeEntries);

  DBGLOG_FORCE(
      force, "Total Blocks  %5i    Used Blocks  %5i    Free Blocks  %5i\n",
...
int umm_usage_metric(void) { return umm_multi_usage_metric(&umm_heap_current); }
