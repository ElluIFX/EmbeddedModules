 ...
 *                        added user-dependent configuration file
 * umm_malloc_cfg.h R.Hempel 2016-12-04 - Add support for Unity test framework
 ...
 * R.Hempel 2020-01-12 - Use explicitly sized values from stdint.h - See Issue
 * 15 R.Hempel 2020-01-20 - Move metric functions back to umm_info - See Issue
 * 29 R.Hempel 2020-02-01 - Macro functions are uppercased - See Issue 34
 ...
 */
static void umm_split_block(umm_heap *heap, uint16_t c, uint16_t blocks,
...
static uint16_t umm_assimilate_down(umm_heap *heap, uint16_t c,
                                    uint16_t freemask) {
...
void umm_multi_init_heap(umm_heap *heap, void *ptr, size_t size) {
  ...
  // DBGLOG_FORCE(true, "nblock(0) %04x pblock(0) %04x nfree(0) %04x pfree(0)
  // %04x\n", UMM_NBLOCK(0) & UMM_BLOCKNO_MASK, UMM_PBLOCK(0), UMM_NFREE(0),
  // UMM_PFREE(0)); DBGLOG_FORCE(true, "nblock(1) %04x pblock(1) %04x nfree(1)
  // %04x pfree(1) %04x\n", UMM_NBLOCK(1) & UMM_BLOCKNO_MASK, UMM_PBLOCK(1),
  // UMM_NFREE(1), UMM_PFREE(1));
}
...
void umm_multi_free(umm_heap *heap, void *ptr) {
  ...
  if ((ptr < heap->pheap) ||
      ((size_t)ptr >= (size_t)heap->pheap + heap->heap_size)) {
...
void *umm_multi_realloc(umm_heap *heap, void *ptr, size_t size) {
  ...
  if (blockSize >= blocks) {
    ...
    if ((ptr = umm_malloc_core(heap, size))) {
      ...
      DBGLOG_DEBUG(
          "realloc %i to a bigger block %i failed - return NULL and leave the "
          "old block!\n",
          blockSize, blocks);
...
void umm_free(void *ptr) { umm_multi_free(&umm_heap_current, ptr); }
