...
#define MODBUS_BUFFER_SIZE \
  ((MODBUS_REGISTER_LIMIT)*4 + 20)  // 数据包最大长度(写多个寄存器的数据包长度)
...
/************ 对外接口 BEGIN ***********/
void ModBus_setup(ModBus_parameter* ModBus_para,
                  ModBus_Setting_T setting);  // 配置ModBus实例
void ModBus_readByteFromOuter(
    ModBus_parameter* ModBus_para,
    uint8_t receivedByte);  // 传递字节数据到ModBus协议
...
*count), 参数包括首地址和寄存器个数,写指令超时传入参数(0,0)
** 返回指令序号(大于0), 以便在回调函数中判断完成的是哪一指令, 不可发送则返回0
***/
