...
void genann_train(genann const* ann, double const* inputs,
    ...
    for (h = ann->hidden_layers - 1; h >= 0; --h) {
        ...
        double const* const ww = ann->weight +
    ...
    /* Train the outputs. */
    {
        ...
        double const* d =
            ann->delta +
            ann->hidden * ann->hidden_layers; /* First output delta. */
        ...
        for (j = 0; j < ann->outputs; ++j) {
            *w++ += *d * learning_rate * -1.0;
            for (k = 1;
                 k < (ann->hidden_layers ? ann->hidden : ann->inputs) + 1;
                 ++k) {
    ...
    for (h = ann->hidden_layers - 1; h >= 0; --h) {
        ...
        double* w =
            ann->weight + (h ? ((ann->inputs + 1) * ann->hidden +
