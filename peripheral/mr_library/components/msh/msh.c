...
#define MR_CFG_MSH_HISTORY_LINES (4)
    ...
    char history[MR_CFG_MSH_HISTORY_LINES]
                [MR_CFG_MSH_BUFSZ]; /**< History buffer */
    char* prompt;                   /**< Prompt string */
    int desc;                       /**< Device descriptor */
} msh;

#define MR_MSH_EXPORT(name, fn, help, level)                       \
    MR_USED const struct mr_msh_cmd _mr_msh_cmd_##name MR_SECTION( \
        "mr_msh_cmd." level) = {#name, fn, help};
...
#define MSH_CLEAR_SCREEN \
    "\x1b[2J"            \
    "\x1b[1;1H" /**< Clear screen */
#define MSH_CLEAR_LINE \
    "\x1b[2K"          \
    "\x1b[1G" /**< Clear line */
...
MR_INLINE void msh_refresh_line(void) {
    if (msh.echo == MR_ENABLE) {
        ...
        mr_msh_printf(MR_MSH_COLOR_CYAN("%s ") MR_MSH_COLOR_YELLOW("%s> ") "%s",
                      MR_CFG_MSH_PROMPT, msh.prompt, msh.buf);
    }
}
...
MR_INLINE void msh_auto_complete(void) {
    ...
    for (const struct mr_msh_cmd* msh_cmd = ((&_mr_msh_cmd_start) + 1);
         msh_cmd < &_mr_msh_cmd_end; msh_cmd++) {
...
MR_INLINE void msh_history_pop_prev(void) {
    if (msh.history_pos < msh.history_len) {
        ...
        uint16_t index =
            (msh.history_index + MR_CFG_MSH_HISTORY_LINES - msh.history_pos) %
            MR_CFG_MSH_HISTORY_LINES;
...
static struct {
    ...
    {KEY_ENTER, msh_key_enter}, {KEY_BACKSPACE, msh_key_backspace},
    {KEY_TABLE, msh_key_table}, {KEY_LEFT, msh_key_left},
    {KEY_RIGHT, msh_key_right}, {KEY_UP, msh_key_up},
    {KEY_DOWN, msh_key_down},   {KEY_DELETE, msh_key_delete},
};
