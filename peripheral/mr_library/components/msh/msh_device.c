...
static void msh_cmd_dopen(int argc, void* argv) {
    ...
    /* Parse [-n] */
    if ((MR_MSH_GET_ARG(3) != MR_NULL) &&
        (strncmp(MR_MSH_GET_ARG(3), "-n", 2) == 0)) {
        flags |= MR_O_NONBLOCK;
    }
...
static void msh_cmd_dclose(int argc, void* argv) {
    ...
    if ((MR_MSH_GET_ARG(1) != MR_NULL) &&
        (strncmp(MR_MSH_GET_ARG(1), "-h", 2) == 0)) {
        goto usage;
    }
...
static void msh_cmd_dwrite(int argc, void* argv) {
    ...
    for (size_t i = data_index;
         i <= (argc < (sizeof(buf) / itemsz + data_index)
                   ? argc
                   : (sizeof(buf) / itemsz + data_index));
         i++) {
        int arg;
