...
MR_INLINE int adc_channel_set_configure(struct mr_adc* adc, int channel,
...
MR_INLINE int adc_channel_get_configure(struct mr_adc* adc, int channel,
                                        struct mr_adc_config* config) {
    if ((channel < 0) || (channel >= (sizeof(adc->channels) * 8))) {
        return MR_EINVAL;
    }
...
static ssize_t mr_adc_read(struct mr_dev* dev, void* buf, size_t count) {
    ...
    if ((dev->position < 0) ||
        (MR_BIT_IS_SET(adc->channels, (1 << dev->position)) == MR_DISABLE)) {
        return MR_EINVAL;
    }
...
int mr_adc_register(struct mr_adc* adc, const char* path, struct mr_drv* drv) {
    static struct mr_dev_ops ops = {mr_adc_open, mr_adc_close, mr_adc_read,
                                    MR_NULL,     mr_adc_ioctl, MR_NULL};
    ...
    /* Register the adc */
    return mr_dev_register(&adc->dev, path, MR_DEV_TYPE_ADC, MR_O_RDONLY, &ops,
                           drv);
}
