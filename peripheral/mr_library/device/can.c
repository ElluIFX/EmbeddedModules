...
static ssize_t mr_can_bus_write(struct mr_dev* dev, const void* buf,
                                size_t count) {
    return MR_EIO;
}
...
static ssize_t mr_can_bus_isr(struct mr_dev* dev, int event, void* args) {
    ...
    switch (event) {
        case MR_ISR_CAN_RD_INT: {
            ...
            ssize_t ret =
                ops->read(can_bus, &id, &ide, &rtr, data, sizeof(data));
            if (ret < 0) {
                return ret;
            }
            ...
            for (struct mr_list* list = dev->clist.next; list != &dev->clist;
                 list = list->next) {
                struct mr_can_dev* can_dev =
                    (struct mr_can_dev*)MR_CONTAINER_OF(list, struct mr_dev,
                                                        list);
 ...
 */
int mr_can_bus_register(struct mr_can_bus* can_bus, const char* path,
                        struct mr_drv* drv) {
    static struct mr_dev_ops ops = {mr_can_bus_open, mr_can_bus_close,
                                    mr_can_bus_read, mr_can_bus_write,
                                    MR_NULL,         mr_can_bus_isr};
    ...
    return mr_dev_register(&can_bus->dev, path, MR_DEV_TYPE_CAN, MR_O_RDWR,
                           &ops, drv);
}

MR_INLINE int can_dev_filter_configure(struct mr_can_dev* can_dev, int id,
                                       int ide, int state) {
...
MR_INLINE ssize_t can_dev_write(struct mr_can_dev* can_dev, int id, int ide,
...
static int mr_can_dev_open(struct mr_dev* dev) {
    ...
    return can_dev_filter_configure(can_dev, can_dev->id, can_dev->ide,
                                    MR_ENABLE);
}
...
static ssize_t mr_can_dev_write(struct mr_dev* dev, const void* buf,
                                size_t count) {
    struct mr_can_dev* can_dev = (struct mr_can_dev*)dev;
    ...
    ret =
        can_dev_write(can_dev, can_dev->id, can_dev->ide, (uint8_t*)buf, count);
 ...
 * @return 0 on success, otherwise an error code.
 */
int mr_can_dev_register(struct mr_can_dev* can_dev, const char* path, int id,
                        int ide) {
    static struct mr_dev_ops ops = {mr_can_dev_open,  mr_can_dev_close,
                                    mr_can_dev_read,  mr_can_dev_write,
                                    mr_can_dev_ioctl, MR_NULL};
    struct mr_can_config default_config = MR_CAN_CONFIG_DEFAULT;
...
#define MR_CFG_CAN_RD_BUFSZ (0)
    ...
    return mr_dev_register(&can_dev->dev, path, MR_DEV_TYPE_CAN, MR_O_RDWR,
                           &ops, MR_NULL);
}
