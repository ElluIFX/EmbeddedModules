...
MR_INLINE int dac_channel_set_configure(struct mr_dac* dac, int channel,
                                        struct mr_dac_config config) {
    struct mr_dac_ops* ops = (struct mr_dac_ops*)dac->dev.drv->ops;

    if ((channel < 0) || (channel >= (sizeof(dac->channels) * 8))) {
        return MR_EINVAL;
    }
    ...
    if (config.state == MR_ENABLE) {
        MR_BIT_SET(dac->channels, (1 << channel));
    } else {
        MR_BIT_CLR(dac->channels, (1 << channel));
    }
    return MR_EOK;
}

MR_INLINE int dac_channel_get_configure(struct mr_dac* dac, int channel,
                                        struct mr_dac_config* config) {
    if ((channel < 0) || (channel >= (sizeof(dac->channels) * 8))) {
        return MR_EINVAL;
    }
    ...
    /* Get configure */
    config->state = MR_BIT_IS_SET(dac->channels, (1 << channel));
    return MR_EOK;
}
...
static int mr_dac_close(struct mr_dev* dev) {
    ...
    for (size_t i = 0; i < (sizeof(dac->channels) * 8); i++) {
        if (MR_BIT_IS_SET(dac->channels, (1 << i)) == MR_ENABLE) {
            ops->channel_configure(dac, (int)i, MR_DISABLE);
            MR_BIT_CLR(dac->channels, (1 << i));
        }
    }
...
static ssize_t mr_dac_write(struct mr_dev* dev, const void* buf, size_t count) {
    ...
    if ((dev->position < 0) ||
        (MR_BIT_IS_SET(dac->channels, (1 << dev->position)) == MR_DISABLE)) {
        return MR_EINVAL;
    }
...
int mr_dac_register(struct mr_dac* dac, const char* path, struct mr_drv* drv) {
    static struct mr_dev_ops ops = {mr_dac_open,  mr_dac_close, MR_NULL,
                                    mr_dac_write, mr_dac_ioctl, MR_NULL};
    ...
    /* Register the dac */
    return mr_dev_register(&dac->dev, path, MR_DEV_TYPE_DAC, MR_O_WRONLY, &ops,
                           drv);
}
