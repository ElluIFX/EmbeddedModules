...
int _mr_fast_pin_mode(int number, int mode) {
    ...
    return ((struct mr_pin_ops*)dev->drv->ops)
        ->configure((struct mr_pin*)dev, number, mode);
}
...
uint8_t _mr_fast_pin_read(int number) {
    ...
    ((struct mr_pin_ops*)dev->drv->ops)
        ->read((struct mr_pin*)dev, number, &value);
    return value;
}
...
void _mr_fast_pin_write(int number, int value) {
    ...
    ((struct mr_pin_ops*)dev->drv->ops)
        ->write((struct mr_pin*)dev, number, value);
}
