...
static ssize_t mr_i2c_bus_write(struct mr_dev* dev, const void* buf,
                                size_t count) {
    return MR_ENOTSUP;
}
 ...
 */
int mr_i2c_bus_register(struct mr_i2c_bus* i2c_bus, const char* path,
                        struct mr_drv* drv) {
    static struct mr_dev_ops ops = {mr_i2c_bus_open, mr_i2c_bus_close,
                                    mr_i2c_bus_read, mr_i2c_bus_write,
                                    MR_NULL,         mr_i2c_bus_isr};
    ...
    return mr_dev_register(&i2c_bus->dev, path, MR_DEV_TYPE_I2C, MR_O_RDWR,
                           &ops, drv);
}
...
MR_INLINE int i2c_dev_take_bus(struct mr_i2c_dev* i2c_dev) {
    ...
    if (i2c_dev != i2c_bus->owner) {
        if (i2c_dev->config.baud_rate != i2c_bus->config.baud_rate ||
            ...
            int addr = (i2c_dev->config.host_slave == MR_I2C_HOST)
                           ? 0x00
                           : i2c_dev->addr;

            int ret = ops->configure(i2c_bus, &i2c_dev->config, addr,
                                     i2c_dev->addr_bits);
            if (ret < 0) {
                return ret;
            }
...
MR_INLINE ssize_t i2c_dev_read(struct mr_i2c_dev* i2c_dev, uint8_t* buf,
                               size_t count) {
    struct mr_i2c_bus* i2c_bus = (struct mr_i2c_bus*)i2c_dev->dev.parent;
    struct mr_i2c_bus_ops* ops = (struct mr_i2c_bus_ops*)i2c_bus->dev.drv->ops;
    ssize_t rd_size;
...
MR_INLINE ssize_t i2c_dev_write(struct mr_i2c_dev* i2c_dev, const uint8_t* buf,
                                size_t count) {
    struct mr_i2c_bus* i2c_bus = (struct mr_i2c_bus*)i2c_dev->dev.parent;
    struct mr_i2c_bus_ops* ops = (struct mr_i2c_bus_ops*)i2c_bus->dev.drv->ops;
    ssize_t wr_size;
...
static ssize_t mr_i2c_dev_read(struct mr_dev* dev, void* buf, size_t count) {
    ...
    if (i2c_dev->config.host_slave == MR_I2C_HOST) {
        ...
        if (dev->position >= 0) {
            ...
            ret = i2c_dev_write(i2c_dev, (uint8_t*)&dev->position,
...
static ssize_t mr_i2c_dev_write(struct mr_dev* dev, const void* buf,
                                size_t count) {
    struct mr_i2c_dev* i2c_dev = (struct mr_i2c_dev*)dev;
 ...
 * @return 0 on success, otherwise an error code.
 */
int mr_i2c_dev_register(struct mr_i2c_dev* i2c_dev, const char* path, int addr,
                        int addr_bits) {
    static struct mr_dev_ops ops = {mr_i2c_dev_open,  mr_i2c_dev_close,
                                    mr_i2c_dev_read,  mr_i2c_dev_write,
                                    mr_i2c_dev_ioctl, MR_NULL};
    struct mr_i2c_config default_config = MR_I2C_CONFIG_DEFAULT;

    MR_ASSERT(i2c_dev != MR_NULL);
    MR_ASSERT(path != MR_NULL);
    MR_ASSERT((addr_bits == MR_I2C_ADDR_BITS_7) ||
              (addr_bits == MR_I2C_ADDR_BITS_10));
    MR_ASSERT((addr_bits != MR_I2C_ADDR_BITS_7) || (addr >= 0 && addr <= 0x7f));
    MR_ASSERT((addr_bits != MR_I2C_ADDR_BITS_10) ||
              (addr >= 0 && addr <= 0x3ff));
...
#define MR_CFG_I2C_RD_BUFSZ (0)
    ...
    return mr_dev_register(&i2c_dev->dev, path, MR_DEV_TYPE_I2C, MR_O_RDWR,
                           &ops, MR_NULL);
}
