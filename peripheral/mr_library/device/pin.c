...
#define PIN_MODE_SET(_pin, _number, _mode)              \
    do {                                                \
        MR_BIT_CLR((_pin)->pins[(_number) / 8],         \
                   (0xf << (((_number) % 8) * 4)));     \
        MR_BIT_SET((_pin)->pins[(_number) / 8],         \
                   ((_mode) << (((_number) % 8) * 4))); \
    } while (0)
#define PIN_MODE_GET(_pin, _number) \
    ((int)(((_pin)->pins[(_number) / 8] >> (((_number) % 8) * 4)) & 0xf))

MR_INLINE int pin_set_mode(struct mr_pin* pin, int number,
...
MR_INLINE int pin_get_mode(struct mr_pin* pin, int number,
                           struct mr_pin_config* config) {
    if ((number < 0) || (number >= (sizeof(pin->pins) * 2))) {
        return MR_EINVAL;
    }
...
static ssize_t mr_pin_read(struct mr_dev* dev, void* buf, size_t count) {
    ...
    if ((dev->position < 0) || (dev->position >= (sizeof(pin->pins) * 2)) ||
...
int mr_pin_register(struct mr_pin* pin, const char* path, struct mr_drv* drv) {
    static struct mr_dev_ops ops = {MR_NULL,      mr_pin_close, mr_pin_read,
                                    mr_pin_write, mr_pin_ioctl, mr_pin_isr};
    ...
    /* Register the pin */
    int ret =
        mr_dev_register(&pin->dev, path, MR_DEV_TYPE_PIN, MR_O_RDWR, &ops, drv);
    if (ret == MR_EOK) {
