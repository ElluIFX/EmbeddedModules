...
MR_INLINE int pwm_channel_set_configure(struct mr_pwm* pwm, int channel,
    ...
    int ret =
        ops->channel_configure(pwm, channel, config.state, config.polarity);
    if (ret < 0) {
        return ret;
    }
...
MR_INLINE int pwm_channel_get_configure(struct mr_pwm* pwm, int channel,
                                        struct mr_pwm_config* config) {
    if ((channel < 0) || (channel >= (sizeof(pwm->channel) * 8))) {
        return MR_EINVAL;
    }
...
MR_INLINE int pwm_calculate(struct mr_pwm* pwm, uint32_t freq) {
    uint32_t clk = pwm->info->clk, psc_max = pwm->info->prescaler_max,
             per_max = pwm->info->period_max;
    ...
    if (product <= per_max) {
        ...
        for (uint32_t psc = MR_BOUND(product / per_max, 1, psc_max);
             psc < psc_max; psc++) {
...
static int mr_pwm_close(struct mr_dev* dev) {
    ...
    for (size_t i = 0; i < (sizeof(pwm->channel) * 8); i++) {
        if (MR_BIT_IS_SET(pwm->channel, (1 << i)) == MR_ENABLE) {
            ops->channel_configure(pwm, (int)i, MR_DISABLE,
                                   MR_PWM_POLARITY_NORMAL);
...
static ssize_t mr_pwm_read(struct mr_dev* dev, void* buf, size_t count) {
    ...
    if ((dev->position < 0) ||
        (MR_BIT_IS_SET(pwm->channel, (1 << dev->position)) == MR_DISABLE)) {
        return MR_EINVAL;
    }
#endif /* MR_USING_PWM_CHANNEL_CHECK */

    for (rd_size = 0; rd_size < MR_ALIGN_DOWN(count, sizeof(*rd_buf));
         rd_size += sizeof(*rd_buf)) {
        ...
        *rd_buf = (uint32_t)(((float)compare_value / (float)pwm->period) *
                             1000000.0f);
        rd_buf++;
    }
    return rd_size;
...
static ssize_t mr_pwm_write(struct mr_dev* dev, const void* buf, size_t count) {
    ...
    for (wr_size = 0; wr_size < MR_ALIGN_DOWN(count, sizeof(*wr_buf));
         wr_size += sizeof(*wr_buf)) {
        /* Calculate the compare value */
        uint32_t compare_value = MR_BOUND(
            (uint32_t)(((float)*wr_buf / 1000000.0f) * (float)(pwm->period)), 0,
            pwm->period);
            ...
            if (args != MR_NULL) {
                ...
                for (size_t i = 0; i < 32; i++) {
                    if (MR_BIT_IS_SET(pwm->channel, (1 << i)) == MR_ENABLE) {
                        ...
                        compare_value = (uint32_t)(((float)compare_value /
                                                    (float)old_period) *
                                                   (float)(pwm->period));
 ...
 */
int mr_pwm_register(struct mr_pwm* pwm, const char* path, struct mr_drv* drv,
                    struct mr_pwm_info* info) {
    static struct mr_dev_ops ops = {mr_pwm_open,  mr_pwm_close, mr_pwm_read,
                                    mr_pwm_write, mr_pwm_ioctl, MR_NULL};
    ...
    /* Register the pwm */
    return mr_dev_register(&pwm->dev, path, MR_DEV_TYPE_PWM, MR_O_RDWR, &ops,
                           drv);
}
