...
MR_INLINE ssize_t serial_poll_read(struct mr_serial* serial, uint8_t* buf,
                                   size_t count) {
...
MR_INLINE ssize_t serial_poll_write(struct mr_serial* serial, uint8_t* buf,
                                    size_t count) {
    struct mr_serial_ops* ops = (struct mr_serial_ops*)serial->dev.drv->ops;
    ssize_t size;
...
MR_INLINE ssize_t serial_dma_write(struct mr_serial* serial, uint8_t* buf,
                                   size_t count) {
    struct mr_serial_ops* ops = (struct mr_serial_ops*)serial->dev.drv->ops;
    ...
    if (serial->dma_wr_bufsz == 0) {
        ...
        if (serial->nonblock_state == MR_DISABLE) {
            if (count > serial->dma_wr_bufsz) {
                ...
                ops->start_dma_tx(serial, serial->dma_wr_buf,
                                  serial->dma_wr_bufsz);
                return (
                    ssize_t)(serial->dma_wr_bufsz +
...
MR_INLINE ssize_t serial_nonblocking_write(struct mr_serial* serial,
...
static ssize_t mr_serial_write(struct mr_dev* dev, const void* buf,
                               size_t count) {
    struct mr_serial* serial = (struct mr_serial*)dev;
        ...
        case MR_IOC_SERIAL_SET_RD_DMA_BUFSZ: {
            if (args != MR_NULL) {
                ...
                if ((ops->start_dma_rx != MR_NULL) &&
                    (serial->dma_rd_bufsz != 0)) {
                    ops->start_dma_rx(serial, serial->dma_rd_buf,
                                      serial->dma_rd_bufsz);
                }
                return sizeof(bufsz);
            }
    ...
    switch (event) {
        ...
        case MR_ISR_SERIAL_RD_DMA: {
            if (args != MR_NULL) {
                ...
                mr_ringbuf_write_force(
                    &serial->rd_fifo, serial->dma_rd_buf,
        ...
        case MR_ISR_SERIAL_WR_DMA: {
            if (serial->dma_wr_bufsz == 0) {
                ...
                size_t size = mr_ringbuf_read(
                    &serial->wr_fifo, serial->dma_wr_buf, serial->dma_wr_bufsz);
                if (size != 0) {
...
int mr_serial_register(struct mr_serial* serial, const char* path,
                       struct mr_drv* drv) {
    static struct mr_dev_ops ops = {mr_serial_open,  mr_serial_close,
                                    mr_serial_read,  mr_serial_write,
                                    mr_serial_ioctl, mr_serial_isr};
...
#define MR_CFG_SERIAL_WR_DMA_BUFSZ (0)
    ...
    /* Register the serial */
    return mr_dev_register(&serial->dev, path, MR_DEV_TYPE_SERIAL,
                           MR_O_RDWR | MR_O_NONBLOCK, &ops, drv);
}
