...
static ssize_t mr_spi_bus_write(struct mr_dev* dev, const void* buf,
                                size_t count) {
    return MR_EIO;
}
...
int mr_spi_bus_register(struct mr_spi_bus* spi_bus, const char* path,
                        struct mr_drv* drv) {
    static struct mr_dev_ops ops = {mr_spi_bus_open, mr_spi_bus_close,
                                    mr_spi_bus_read, mr_spi_bus_write,
                                    MR_NULL,         mr_spi_bus_isr};
    ...
    return mr_dev_register(&spi_bus->dev, path, MR_DEV_TYPE_SPI, MR_O_RDWR,
                           &ops, drv);
}
    ...
    if (spi_dev->cs_pin != -1) {
        ...
        if (state == MR_ENABLE) {
            if (spi_dev->config.host_slave == MR_SPI_HOST) {
                mr_dev_ioctl(desc, MR_IOC_SCFG,
                             MR_MAKE_LOCAL(int, MR_PIN_MODE_OUTPUT));
                mr_dev_write(desc, MR_MAKE_LOCAL(uint8_t, !spi_dev->cs_active),
                             sizeof(uint8_t));
            } else {
                mr_dev_ioctl(desc, MR_IOC_SCFG,
                             MR_MAKE_LOCAL(int, MR_PIN_MODE_NONE));
            }
        } else {
            mr_dev_ioctl(desc, MR_IOC_SCFG,
                         MR_MAKE_LOCAL(int, MR_PIN_MODE_NONE));
...
#ifdef MR_USING_PIN
        if ((spi_bus->cs_desc >= 0) &&
            (spi_bus->config.host_slave == MR_SPI_HOST)) {
            mr_dev_ioctl(spi_bus->cs_desc, MR_IOC_SPOS,
                         MR_MAKE_LOCAL(int, spi_dev->cs_pin));
        }
#endif /* MR_USING_PIN */
    }
...
static ssize_t spi_dev_transfer(struct mr_spi_dev* spi_dev, uint8_t* rd_buf,
                                const uint8_t* wr_buf, size_t size, int rdwr) {
    struct mr_spi_bus* spi_bus = (struct mr_spi_bus*)spi_dev->dev.parent;
    struct mr_spi_bus_ops* ops = (struct mr_spi_bus_ops*)spi_bus->dev.drv->ops;
    ssize_t tf_size;
    ...
    if (rdwr == MR_SPI_RD) {
        ...
        for (tf_size = 0; tf_size < MR_ALIGN_DOWN(size, sizeof(*wr_buf));
...
static ssize_t mr_spi_dev_read(struct mr_dev* dev, void* buf, size_t count) {
    ...
    if (spi_dev->config.host_slave == MR_SPI_HOST) {
        ...
        if (dev->position >= 0) {
            ret = spi_dev_transfer(spi_dev, MR_NULL, (uint8_t*)&dev->position,
                                   (spi_dev->config.reg_bits >> 3), MR_SPI_WR);
            if (ret < 0) {
...
static ssize_t mr_spi_dev_write(struct mr_dev* dev, const void* buf,
                                size_t count) {
    struct mr_spi_dev* spi_dev = (struct mr_spi_dev*)dev;
    ...
    switch (cmd) {
        ...
        case MR_IOC_SPI_TRANSFER: {
            if (args != MR_NULL) {
                ...
                if (spi_dev->config.host_slave == MR_SPI_HOST) {
                    spi_dev_cs_set(spi_dev, MR_ENABLE);
                    ret = (int)spi_dev_transfer(dev->parent, transfer.rd_buf,
                                                transfer.wr_buf, transfer.size,
                                                MR_SPI_RDWR);
                    spi_dev_cs_set(spi_dev, MR_DISABLE);
                } else {
                    ret = (int)spi_dev_transfer(dev->parent, transfer.rd_buf,
 ...
 * @return 0 on success, otherwise an error code.
 */
int mr_spi_dev_register(struct mr_spi_dev* spi_dev, const char* path,
                        int cs_pin, int cs_active) {
    static struct mr_dev_ops ops = {mr_spi_dev_open,  mr_spi_dev_close,
                                    mr_spi_dev_read,  mr_spi_dev_write,
                                    mr_spi_dev_ioctl, MR_NULL};
    struct mr_spi_config default_config = MR_SPI_CONFIG_DEFAULT;

    MR_ASSERT(spi_dev != MR_NULL);
    MR_ASSERT(path != MR_NULL);
    MR_ASSERT((cs_active >= MR_SPI_CS_ACTIVE_LOW) &&
              (cs_active <= MR_SPI_CS_ACTIVE_HARDWARE));
...
#define MR_CFG_SPI_RD_BUFSZ (0)
    ...
    return mr_dev_register(&spi_dev->dev, path, MR_DEV_TYPE_SPI, MR_O_RDWR,
                           &ops, MR_NULL);
}
