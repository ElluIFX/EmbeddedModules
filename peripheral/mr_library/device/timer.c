...
static int timer_calculate(struct mr_timer* timer, uint32_t timeout) {
    uint32_t clk = timer->info->clk, psc_max = timer->info->prescaler_max,
             per_max = timer->info->period_max;
    ...
    if (product <= per_max) {
        ...
        for (uint32_t psc = MR_BOUND((product / per_max), 1, product);
             psc < UINT32_MAX; psc++) {
...
static ssize_t mr_timer_read(struct mr_dev* dev, void* buf, size_t count) {
    ...
    for (rd_size = 0; rd_size < MR_ALIGN_DOWN(count, sizeof(*rd_buf));
        ...
        *rd_buf = (timer->reload - timer->count) * timer->timeout +
                  (uint32_t)(((float)cnt / (float)timer->period) *
                             (float)timer->timeout);
        rd_buf++;
    }
    return rd_size;
...
static ssize_t mr_timer_write(struct mr_dev* dev, const void* buf,
                              size_t count) {
    struct mr_timer* timer = (struct mr_timer*)dev;
    struct mr_timer_ops* ops = (struct mr_timer_ops*)dev->drv->ops;
 ...
 */
int mr_timer_register(struct mr_timer* timer, const char* path,
                      struct mr_drv* drv, struct mr_timer_info* info) {
    static struct mr_dev_ops ops = {mr_timer_open,  mr_timer_close,
                                    mr_timer_read,  mr_timer_write,
                                    mr_timer_ioctl, mr_timer_isr};
    ...
    /* Register the timer */
    return mr_dev_register(&timer->dev, path, MR_DEV_TYPE_TIMER, MR_O_RDWR,
                           &ops, drv);
}
