...
#if !defined(MR_USING_DAC1) && !defined(MR_USING_DAC2) && \
    !defined(MR_USING_DAC3)
...
static int drv_dac_channel_configure(struct mr_dac* dac, int channel,
                                     int state) {
    struct drv_dac_data* dac_data = (struct drv_dac_data*)dac->dev.drv->data;
    struct drv_dac_channel_data* dac_channel_data =
        drv_dac_get_channel_data(channel);
    ...
    HAL_DAC_ConfigChannel(&dac_data->handle, &sConfig,
                          dac_channel_data->channel);
    if (state == ENABLE) {
        HAL_DAC_SetValue(&dac_data->handle, dac_channel_data->channel,
                         DAC_ALIGN_12B_R, 0);
