...
static struct drv_pin_port_data* drv_pin_get_port_data(int pin) {
    ...
    if ((pin >= MR_ARRAY_NUM(pin_port_drv_data)) ||
        (pin_port_drv_data[pin].port == MR_NULL)) {
        return MR_NULL;
    }
...
static int drv_pin_configure(struct mr_pin* pin, int number, int mode) {
    ...
    if (mode >= MR_PIN_MODE_IRQ_RISING) {
        ...
        if ((exti_line >= 5) && (exti_line <= 9)) {
            if ((pin_irq_mask[5] == -1) && (pin_irq_mask[6] == -1) &&
                (pin_irq_mask[7] == -1) && (pin_irq_mask[8] == -1) &&
                (pin_irq_mask[9] == -1)) {
                HAL_NVIC_DisableIRQ(pin_irq_map[exti_line]);
            }
        } else {
            if ((pin_irq_mask[10] == -1) && (pin_irq_mask[11] == -1) &&
                (pin_irq_mask[12] == -1) && (pin_irq_mask[13] == -1) &&
                (pin_irq_mask[14] == -1) && (pin_irq_mask[15] == -1)) {
                HAL_NVIC_DisableIRQ(pin_irq_map[exti_line]);
            }
        }
...
static struct mr_pin_ops pin_drv_ops = {drv_pin_configure, drv_pin_read,
                                        drv_pin_write};

static struct mr_drv pin_drv = {&pin_drv_ops, MR_NULL};
