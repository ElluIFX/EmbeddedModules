...
#include "drv_spi.h"
    ...
    !defined(MR_USING_SPI3) && !defined(MR_USING_SPI4) && \
...
static int drv_spi_bus_configure(struct mr_spi_bus* spi_bus,
                                 struct mr_spi_config* config) {
    struct drv_spi_bus_data* spi_bus_data =
        (struct drv_spi_bus_data*)spi_bus->dev.drv->data;
    ...
    if (state == ENABLE) {
        ...
        if (psc >= 256) {
            spi_bus_data->handle.Init.BaudRatePrescaler =
...
static void drv_spi_bus_isr(struct mr_spi_bus* spi_bus) {
    ...
    if ((__HAL_SPI_GET_FLAG(&spi_bus_data->handle, SPI_FLAG_RXNE) != RESET) &&
        (__HAL_SPI_GET_IT_SOURCE(&spi_bus_data->handle, SPI_IT_RXNE) !=
         RESET)) {
