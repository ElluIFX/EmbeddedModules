...
MR_INLINE struct mr_dev* dev_find_child(struct mr_dev* parent,
                                        const char* name) {
    for (struct mr_list* list = parent->clist.next; list != &parent->clist;
         list = list->next) {
        struct mr_dev* dev =
            (struct mr_dev*)MR_CONTAINER_OF(list, struct mr_dev, list);
...
MR_INLINE int dev_register_child(struct mr_dev* parent, struct mr_dev* child,
                                 const char* name) {
...
static int dev_register_by_path(struct mr_dev* parent, struct mr_dev* dev,
                                const char* path) {
    if (path[0] == '/') {
        path++;
    }
...
static struct mr_dev* dev_find_by_path(struct mr_dev* parent,
                                       const char* path) {
    if (path[0] == '/') {
        path++;
    }
...
MR_INLINE struct mr_dev* dev_find(const char* path) {
    ...
    if (*path == '/') {
        ...
        if ((next_slash == MR_NULL) ||
            (strncmp(path, root_dev.name,
                     MR_BOUND(next_slash - path, 0, MR_CFG_DEV_NAME_LEN)) !=
             0)) {
            return MR_NULL;
        }
...
MR_INLINE ssize_t dev_read(struct mr_dev* dev, int position, int sync,
...
MR_INLINE ssize_t dev_write(struct mr_dev* dev, int position, int sync,
                            const void* buf, size_t count) {
#ifdef MR_USING_RDWR_CTL
    do {
        mr_interrupt_disable();
        int ret = dev_lock_take(dev,
                                (MR_LOCK_WR | MR_LOCK_SLEEP |
...
MR_INLINE int dev_ioctl(struct mr_dev* dev, int position, int sync, int cmd,
                        void* args) {
    ...
    do {
        ...
        if (cmd) {
            mr_interrupt_disable();
            int ret = dev_lock_take(
                dev, (MR_LOCK_RDWR | MR_LOCK_SLEEP | MR_LOCK_NONBLOCK),
                MR_LOCK_RDWR);
            if (ret < 0) {
                mr_interrupt_enable();
                return ret;
 ...
 */
int mr_dev_register(struct mr_dev* dev, const char* path, int type, int flags,
                    struct mr_dev_ops* ops, struct mr_drv* drv) {
    ...
    MR_ASSERT((ops->read != MR_NULL) ||
              (MR_BIT_IS_SET(flags, MR_O_RDONLY) == MR_DISABLE));
    MR_ASSERT((ops->write != MR_NULL) ||
              (MR_BIT_IS_SET(flags, MR_O_WRONLY) == MR_DISABLE));
    ...
    switch (event & MR_ISR_MASK) {
        case MR_ISR_RD: {
            for (struct mr_list* list = dev->rd_call_list.next;
                 list != &dev->rd_call_list; list = list->next) {
                struct mr_dev_desc* desc = (struct mr_dev_desc*)MR_CONTAINER_OF(
                    list, struct mr_dev_desc, rd_call.list);
        ...
        case MR_ISR_WR: {
            ...
            for (struct mr_list* list = dev->wr_call_list.next;
                ...
                struct mr_dev_desc* desc = (struct mr_dev_desc*)MR_CONTAINER_OF(
                    list, struct mr_dev_desc, wr_call.list);
...
ssize_t mr_dev_read(int desc, void* buf, size_t count) {
    ...
    /* Read buffer from the device */
    return dev_read(DESC_OF(desc).dev, DESC_OF(desc).position,
                    MR_BIT_IS_SET(DESC_OF(desc).flags, MR_O_NONBLOCK), buf,
                    count);
}
...
ssize_t mr_dev_write(int desc, const void* buf, size_t count) {
    ...
    /* Write buffer to the device */
    return dev_write(DESC_OF(desc).dev, DESC_OF(desc).position,
                     MR_BIT_IS_SET(DESC_OF(desc).flags, MR_O_NONBLOCK), buf,
                     count);
}
...
int mr_dev_ioctl(int desc, int cmd, void* args) {
    ...
    switch (cmd) {
        ...
        default: {
            /* I/O control to the device */
            return dev_ioctl(DESC_OF(desc).dev, DESC_OF(desc).position,
...
void msh_dlist_tree(struct mr_dev* parent, int level) {
    if (level == 0) {
        mr_msh_printf("|%s %-*s", mr_strflags(parent->flags),
                      MR_CFG_DEV_NAME_LEN, parent->name);
    } else {
        mr_msh_printf("%*s|%s %-*s", level, " ", mr_strflags(parent->flags),
                      MR_CFG_DEV_NAME_LEN, parent->name);
    }
    for (size_t i = 0; i < MR_CFG_DESC_NUM; i++) {
    ...
    mr_msh_printf("\r\n");
    for (struct mr_list* child = parent->clist.next; child != &parent->clist;
         child = child->next) {
