...
#else
        int ret =
            mr_dev_open(MR_CFG_PRINTF_DEV_NAME, MR_O_RDWR | MR_O_NONBLOCK);
...
size_t mr_ringbuf_read(struct mr_ringbuf* ringbuf, void* buffer, size_t size) {
    ...
    memcpy(read_buffer, &ringbuf->buffer[ringbuf->read_index],
           ringbuf->size - ringbuf->read_index);
    memcpy(&read_buffer[ringbuf->size - ringbuf->read_index],
           &ringbuf->buffer[0], size - (ringbuf->size - ringbuf->read_index));
 ...
 * @return The size of the actual write.
 */
size_t mr_ringbuf_write(struct mr_ringbuf* ringbuf, const void* buffer,
                        size_t size) {
    ...
    memcpy(&ringbuf->buffer[ringbuf->write_index], write_buffer,
 ...
 * @return The size of the actual write.
 */
size_t mr_ringbuf_write_force(struct mr_ringbuf* ringbuf, const void* buffer,
                              size_t size) {
    uint8_t* write_buffer = (uint8_t*)buffer;
...
static int mr_avl_get_balance(struct mr_avl* node) {
    ...
    return (mr_avl_get_height(node->left_child) -
            mr_avl_get_height(node->right_child));
}
...
static void mr_avl_left_rotate(struct mr_avl** node) {
    ...
    right_child->height = MR_MAX(mr_avl_get_height(right_child->left_child),
                                 mr_avl_get_height(right_child->right_child)) +
                          1;
    (*node) = right_child;
}
...
static void mr_avl_right_rotate(struct mr_avl** node) {
    ...
    left_child->height = MR_MAX(mr_avl_get_height(left_child->left_child),
                                mr_avl_get_height(left_child->right_child)) +
                         1;
    (*node) = left_child;
}
...
void mr_avl_insert(struct mr_avl** tree, struct mr_avl* node) {
    ...
    (*tree)->height = MR_MAX(mr_avl_get_height((*tree)->left_child),
                             mr_avl_get_height((*tree)->right_child)) +
                      1;
