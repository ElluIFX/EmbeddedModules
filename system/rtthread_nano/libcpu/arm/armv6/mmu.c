...
void mmu_setttbase(rt_uint32_t i) {
    ...
    value = 0x55555555;
    __asm volatile {
        mcr p15, 0, value, c3, c0, 0 mcr p15, 0, i, c2, c0, 0
    }
}
...
void mmu_enable() {
    ...
    __asm volatile {
        mrc p15, 0, value, c1, c0, 0 orr value, value, #0x01 mcr p15, 0, value,
            c1, c0, 0
    }
}
...
void mmu_disable() {
    ...
    __asm volatile {
        mrc p15, 0, value, c1, c0, 0 bic value, value, #0x01 mcr p15, 0, value,
            c1, c0, 0
    }
}
...
void mmu_enable_icache() {
    ...
    __asm volatile {
        mrc p15, 0, value, c1, c0, 0 orr value, value, #0x1000 mcr p15, 0,
            value, c1, c0, 0
    }
}
...
void mmu_enable_dcache() {
    ...
    __asm volatile {
        mrc p15, 0, value, c1, c0, 0 orr value, value, #0x04 mcr p15, 0, value,
            c1, c0, 0
    }
}
...
void mmu_disable_icache() {
    ...
    __asm volatile {
        mrc p15, 0, value, c1, c0, 0 bic value, value, #0x1000 mcr p15, 0,
            value, c1, c0, 0
    }
}
...
void mmu_disable_dcache() {
    ...
    __asm volatile {
        mrc p15, 0, value, c1, c0, 0 bic value, value, #0x04 mcr p15, 0, value,
            c1, c0, 0
    }
}
...
void mmu_enable_alignfault() {
    ...
    __asm volatile {
        mrc p15, 0, value, c1, c0, 0 orr value, value, #0x02 mcr p15, 0, value,
            c1, c0, 0
    }
}
...
void mmu_disable_alignfault() {
    ...
    __asm volatile {
        mrc p15, 0, value, c1, c0, 0 bic value, value, #0x02 mcr p15, 0, value,
            c1, c0, 0
    }
}
...
void mmu_clean_invalidated_dcache(rt_uint32_t buffer, rt_uint32_t size) {
    ...
    while (ptr < buffer + size) {
        __asm volatile {MCR p15, 0, ptr, c7, c14, 1} ptr += CACHE_LINE_SIZE;
    }
}
...
void mmu_clean_dcache(rt_uint32_t buffer, rt_uint32_t size) {
    ...
    while (ptr < buffer + size) {
        __asm volatile {MCR p15, 0, ptr, c7, c10, 1} ptr += CACHE_LINE_SIZE;
    }
}
...
void mmu_invalidate_dcache(rt_uint32_t buffer, rt_uint32_t size) {
    ...
    while (ptr < buffer + size) {
        __asm volatile {MCR p15, 0, ptr, c7, c6, 1} ptr += CACHE_LINE_SIZE;
    }
}
...
void mmu_enable() {
    ...
    i |=
        (1
         << 13); /* High exception vectors selected, address range = 0xFFFF0000-0xFFFF001C */
...
void mmu_create_pte(struct mem_desc* mdesc) {
    ...
    for (i = 0; i < total_page; i++) {
        ...
        if (*pTT ==
            0) /* Level 1 page table item not used, now update pgd item */
        {
            *pTT = pte_offset | sect_attr;
            p_pteentry =
                (rt_uint32_t*)pte_offset + ((vaddr & 0x000ff000) >> 12);
            pte_offset += 1024;
        ...
        *p_pteentry =
            mdesc->page_attr | (((mdesc->paddr_start >> 12) + i) << 12);
        vaddr += 0x1000;
    }
}
...
static void build_pte_mem_desc(struct mem_desc* mdesc, rt_uint32_t size) {
    ...
    for (; size > 0; size--) {
        if (mdesc->mapped_mode == PAGE_MAPPED) {
            nsec = (RT_ALIGN(mdesc->vaddr_end, 0x100000) -
                    RT_ALIGN_DOWN(mdesc->vaddr_start, 0x100000)) >>
                   20;
            mdesc->sect_attr |=
                (((rt_uint32_t)_pte_table) & 0xfffffc00) + pte_offset;
            pte_offset += nsec << 10;
        }
