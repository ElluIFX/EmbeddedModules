...
rt_inline int _has_defunct_thread(void) {
  /* The rt_list_isempty has prototype of "int rt_list_isempty(const rt_list_t
   * *l)". So the compiler has a good reason that the rt_thread_defunct list
   * does not change within rt_thread_idle_excute thus optimize the "while" loop
   * into a "if".
   *
...
void rt_thread_idle_init(void) {
  /* initialize thread */
  rt_thread_init(&idle, "tidle", rt_thread_idle_entry, RT_NULL,
 ...
 *
 */
rt_thread_t rt_thread_idle_gethandler(void) { return (rt_thread_t)(&idle); }
