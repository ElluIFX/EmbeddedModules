...
rt_err_t rt_memheap_init(struct rt_memheap* memheap, const char* name,
    ...
    item->next =
        (struct rt_memheap_item*)((rt_uint8_t*)item + memheap->available_size +
                                  RT_MEMHEAP_SIZE);
    item->prev = item->next;
...
void* rt_memheap_alloc(struct rt_memheap* heap, rt_size_t size) {
    ...
    RT_DEBUG_LOG(RT_DEBUG_MEMHEAP, ("allocate %d on heap:%8.*s", size,
    ...
    if (size < heap->available_size) {
        ...
        if (free_size >= size) {
            ...
            if (free_size >= (size + RT_MEMHEAP_SIZE + RT_MEMHEAP_MINIALLOC)) {
                ...
                /* split the block. */
                new_ptr = (struct rt_memheap_item*)(((rt_uint8_t*)header_ptr) +
                                                    size + RT_MEMHEAP_SIZE);

                RT_DEBUG_LOG(
                    RT_DEBUG_MEMHEAP,
                    ("split: block[0x%08x] nextm[0x%08x] prevm[0x%08x] to "
                     "new[0x%08x]\n",
                ...
                if (heap->pool_size - heap->available_size >
                    heap->max_used_size)
                    heap->max_used_size =
                        heap->pool_size - heap->available_size;
            } else {
                ...
                RT_DEBUG_LOG(
                    RT_DEBUG_MEMHEAP,
                    ("one block: block[0x%08x], next_free 0x%08x, prev_free "
                     "0x%08x\n",
...
void* rt_memheap_realloc(struct rt_memheap* heap, void* ptr,
                         rt_size_t newsize) {
    rt_err_t result;
    rt_size_t oldsize;
    struct rt_memheap_item* header_ptr;
    ...
    if (newsize > oldsize) {
        ...
        if (!RT_MEMHEAP_IS_USED(next_ptr)) {
            ...
            if (nextsize + oldsize > newsize + RT_MEMHEAP_MINIALLOC) {
                /* decrement the entire free size from the available bytes count. */
                heap->available_size =
                    heap->available_size - (newsize - oldsize);
                if (heap->pool_size - heap->available_size >
                    heap->max_used_size)
                    heap->max_used_size =
                        heap->pool_size - heap->available_size;
                ...
                RT_DEBUG_LOG(
                    RT_DEBUG_MEMHEAP,
                    ("remove block: block[0x%08x], next_free 0x%08x, prev_free "
                     "0x%08x",
                ...
                RT_DEBUG_LOG(RT_DEBUG_MEMHEAP,
                             ("new free block: block[0x%08x] nextm[0x%08x] "
                              "prevm[0x%08x]",
...
void rt_system_heap_init(void* begin_addr, void* end_addr) {
    ...
    rt_memheap_init(&_heap, "heap", begin_addr,
...
void* rt_malloc(rt_size_t size) {
    ...
    if (ptr == RT_NULL) {
        ...
        for (node = information->object_list.next;
            ...
            RT_ASSERT(heap);
            RT_ASSERT(rt_object_get_type(&heap->parent) ==
