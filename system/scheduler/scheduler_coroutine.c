...
uint8_t Sch_CreateCortn(const char *name, sch_func_t func, uint8_t enable,
  ...
  p->args = args;
  p->name = name;
  ulist_init(&p->hd.dataList, sizeof(__cortn_data_t), 0,
...
uint8_t Sch_DeleteCortn(const char *name) {
  ...
  ulist_delete(&cortnlist, ulist_index(&cortnlist, cortn));
  cr_modified = 1;
  return 1;
}
...
uint8_t Sch_SetCortnState(const char *name, uint8_t enable,
  ...
  if (clearState) {
    ...
    ulist_clear(&cortn->hd.dataList);
    ulist_append(&cortn->hd.dataList);
...
uint8_t Sch_IsCortnExist(const char *name) { return get_cortn(name) != NULL; }
...
_STATIC_INLINE scheduler_cortn_mutex_t *get_mutex(const char *name) {
  ...
  ret->name = name;
  ret->locked = 0;
  ulist_init(&ret->waitlist, sizeof(char *), 0,
             ULIST_CFG_CLEAR_DIRTY_REGION | ULIST_CFG_NO_ALLOC_EXTEND, NULL);
  return ret;
}
...
void release_barrier(scheduler_cortn_barrier_t *barrier) {
 ...
 */
uint8_t __Sch_CrWaitBarrier(const char *name) {
  scheduler_cortn_barrier_t *barrier = get_barrier(name, 1);
  if (barrier == NULL) return 0;
