 ...
 */
extern uint8_t sch_create_event(const char *name, event_func_t callback,
                                uint8_t enable);
 ...
 * @retval uint8_t          是否成功
 */
extern uint8_t sch_delete_event(const char *name);
 ...
 * @retval uint8_t          是否成功
 */
extern uint8_t sch_set_event_enabled(const char *name, uint8_t enable);
 ...
 */
extern uint8_t sch_get_event_enabled(const char *name);
 ...
 * @note 对于短生命周期的参数, 可以考虑使用sch_trigger_event_ex
 */
extern uint8_t sch_trigger_event(const char *name, uint8_t arg_type,
 ...
 */
extern uint8_t sch_trigger_event_ex(const char *name, uint8_t arg_type,
                                    const void *arg_ptr, size_t arg_size);

/**
 * @brief 获取调度器内事件数量
 */
extern uint16_t sch_get_event_num(void);
 ...
 */
extern uint8_t sch_get_event_exist(const char *name);
