...
tr_res_t tregex_match_str(const char* _srcstr, uint32_t _slen,
...
#endif
  uint8_t misscount = 0, checkmode = TR_MODE_NULL, precheckmode = TR_MODE_NULL;
  ...
  while (p_index <= _plen) {
    switch (_pattern[p_index]) {
      case '^': {
        if (p_index != 0 ||
            (curlevel != 0 && __checkCharNum(&_pattern[p_index], p_index, '(',
                                             1, 1) != curlevel)) {
          goto end;
        }
        s_index = 0;
      ...
      case '[': {
        ...
        for (; p_index < _plen; p_index++) {
          if (_pattern[p_index] == ']' && _pattern[p_index - 1] != '\\') {
            break;
          }
          len_subitem++;
        }
      ...
      case '{': {
        p_index += __getRepeatByBraces((const char*)&_pattern[p_index],
    ...
    switch (checkmode) {
      case TR_MODE_NORMAL: {
        ...
        while (s_index < _slen) {
          if (prechar == _srcstr[s_index]) {
            ...
            misscount = 0;
            break;
          } else if (startpos != NULL || (flag & TINYREGEX_FLAG_STAT) != 0) {
            misscount++;
            break;
          }
          s_index++;
      ...
      case TR_MODE_GLOBBING: {
        while (s_index < _slen) {
          if (__checkCharacterWithGlobbing(_srcstr[s_index], specchar) == 0) {
            if (startpos == NULL) {
      ...
      case TR_MODE_GLOBBING_OPT: {
        while (s_index < _slen) {
          if (__checkOneOfThem(_srcstr[s_index], subitem, len_subitem) == 0) {
            if (startpos == NULL) {
              startpos = &_srcstr[s_index];
            }
            s_index++;
          ...
          if ((flag & TINYREGEX_FLAG_ITEM) != 0) {
            ...
            while (s_index < _slen &&
              ...
              if (precheckmode == TR_MODE_NORMAL) {
                ...
                if (__checkCharacterWithGlobbing(_srcstr[s_index], specchar) !=
                    0) {
                  break;
                }
              } else if (precheckmode == TR_MODE_GLOBBING_OPT) {
                if (__checkOneOfThem(_srcstr[s_index], subitem, len_subitem) !=
                    0) {
                  break;
                }
              }
    ...
    if (misscount > 1) {
      ...
      if (curlevel > 0) {
        p_index -= 2;
        p_index += __getItemEndPos(&_pattern[p_index], _plen - p_index) + 1;
        flag |= TINYREGEX_FLAG_ITEM;
        curlevel--;
        misscount = 1;
