...
int cotMenu_Enter(void) {
  ...
  for (i = 0; i < COT_MENU_SUPPORT_LANGUAGE; i++) {
    pNewMenuCtrl->pszDesc[i] =
        (char *)pCurrMenuCtrl->pMenuList[pCurrMenuCtrl->selectItem].pszDesc[i];
  }
...
int cotMenu_Exit(bool isReset) {
  ...
  if (sg_tMenuManage.pMenuCtrl->pMenuList[sg_tMenuManage.pMenuCtrl->selectItem]
          .pfnExitCallFun != NULL) {
    sg_tMenuManage.pMenuCtrl->isSelected = false;
    sg_tMenuManage.pMenuCtrl->pMenuList[sg_tMenuManage.pMenuCtrl->selectItem]
        .pfnExitCallFun();
  }
