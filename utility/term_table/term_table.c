...
_INTERNAL void tt_check_fmt(TT_FMT1 fmt1, TT_FMT2 fmt2) {
    if (fmt1 == TT_FMT1_KEEP)
        fmt1 = lastFmt1;
    if (fmt2 == TT_FMT2_KEEP)
        fmt2 = lastFmt2;
    if (fmt1 == lastFmt1 && fmt2 == lastFmt2)
        return;
    ...
    if (width < minWidth) {
        switch (str->align) {
            ...
            case TT_ALIGN_CENTER:
                for (int16_t i = 0; i < (minWidth - width) / 2; i++)
                    tt_putchar(sep);
                break;
            case TT_ALIGN_RIGHT:
                for (int16_t i = 0; i < minWidth - width; i++)
                    tt_putchar(sep);
                break;
        }
    }
...
_INTERNAL int16_t tt_calc_kvpair_width(TT_ITEM_KVPAIR kvpair) {
    ...
    ulist_foreach(kvpair->items, term_table_item_kvpair_item_t, kvpair_item) {
        ...
        if (kvpair_item->intent > maxIntent)
            maxIntent = kvpair_item->intent;
    }
...
_INTERNAL int16_t tt_calc_grid_width(TT_ITEM_GRID grid, int16_t minWidth) {
    ...
    ulist_foreach(grid->lines, term_table_item_grid_line_t, grid_line) {
        ...
        ulist_foreach(grid_line->items, term_table_item_grid_item_t,
                      grid_item) {
            if (grid_item->str->width > *width)
                *width = grid_item->str->width;
            width++;
        }
    }
    int16_t sum = 0;
    ...
    ulist_foreach(tt->items, term_table_item_t, item) {
        ...
        switch (item->type) {
            ...
            case TT_ITEM_TYPE_KVPAIR:
                ...
                break;
            case TT_ITEM_TYPE_GRID:
                itemWidth = tt_calc_grid_width((TT_ITEM_GRID)item->content,
                                               tt->tableMinWidth);
                break;
            case TT_ITEM_TYPE_PROGRESS:
...
_INTERNAL void tt_print_separator(TT tt, TT_ITEM_SEPARATOR separator,
    ...
    for (int16_t i = 0; i < width; i++)
        tt_putchar(separator->separator);
    tt_puts(line_break);
...
_INTERNAL void tt_print_kvpair(TT tt, TT_ITEM_KVPAIR kvpair, int16_t minWidth) {
    ulist_foreach(kvpair->items, term_table_item_kvpair_item_t, kvpair_item) {
        if (kvpair_item->intent) {
            for (int16_t i = 0; i < kvpair_item->intent; i++)
                tt_putchar(' ');
        }
...
_INTERNAL void tt_print_grid(TT tt, TT_ITEM_GRID grid, int16_t minWidth) {
    ulist_foreach(grid->lines, term_table_item_grid_line_t, grid_line) {
        if (grid->margin) {
            for (int16_t i = 0; i < grid->margin; i++)
                tt_putchar(' ');
        }
        int16_t* width = (int16_t*)ulist_get(grid->widths, 0);
        ulist_foreach(grid_line->items, term_table_item_grid_item_t,
...
_INTERNAL void tt_print_progress(TT tt, TT_ITEM_PROGRESS progress,
    ...
    if (progress->showPercent) {
        ...
        progress->_percentStr =
            TT_FmtStr(TT_ALIGN_LEFT, TT_FMT1_KEEP, TT_FMT2_KEEP, "%.1f%%",
                      progress->percent * 100.0f);
        width -= progress->_percentStr->width;  // [ ...100.0% ...]
    }
    for (int16_t i = 0; i < progress->intent; i++)
        tt_putchar(' ');
    tt_puts("[");
    int16_t filled = progress->percent * width;
    int16_t empty = width - filled;
    for (int16_t i = 0; i < filled; i++)
        tt_putchar('#');
    for (int16_t i = 0; i < empty; i++)
        tt_putchar(' ');
    if (progress->showPercent) {
    ...
    ulist_foreach(tt->items, term_table_item_t, item) {
        switch (item->type) {
            ...
            case TT_ITEM_TYPE_GRID:
                ...
                break;
            case TT_ITEM_TYPE_SEPARATOR:
                tt_print_separator(tt, (TT_ITEM_SEPARATOR)item->content,
                                   maxWidth);
                break;
            case TT_ITEM_TYPE_LINEBREAK:
                ...
                break;
            case TT_ITEM_TYPE_PROGRESS:
                tt_print_progress(tt, (TT_ITEM_PROGRESS)item->content,
                                  maxWidth);
                break;
        }
    }
